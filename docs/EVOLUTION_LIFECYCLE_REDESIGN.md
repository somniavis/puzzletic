...
> [!IMPORTANT]
> **팻룸 중심 수동 트리거 (Pet Room Only Button)**
> 이제 게임 결과 화면이나 다른 페이지에서 경험치를 채우더라도 즉시 진화하지 않습니다. 사용자가 **팻룸으로 돌아와 벽면의 [진화/졸업] 버튼을 직접 클릭해야** 진화가 시작됩니다. 이를 통해 팝업 스팸을 막고 유저가 원하는 타이밍에 성장을 즐길 수 있습니다.
...
*   **동작**:
    ...
    *   **사용자 액션**: 아이콘 클릭 시 진화 애니메이션 재생 -> 완료 후 Stage 상승.
    ...
    > [!NOTE]
    > **게스트 유저 제한 (Guest Restriction)**
    > 게스트 유저는 Stage 1 -> 2 진화를 수행할 수 없습니다. 진화 버튼 클릭 시 **회원가입 유도 팝업(`SignupPromoModal`)**이 뜨며, 가입 후 데이터를 이관해야만 진화가 가능합니다.
