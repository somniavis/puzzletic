# Profile Page Implementation Guide

This document contains all necessary code and configuration to replicate the Profile Page feature in another project version.

---

## File Structure

```
src/
├── pages/
│   ├── ProfilePage.tsx      # Main Component (NEW)
│   └── ProfilePage.css      # Styling (NEW)
├── i18n/
│   └── locales/
│       └── en.ts            # Translation keys (MODIFY)
└── App.tsx                  # Route configuration (MODIFY)
```

---

## 1. ProfilePage.tsx

Create `src/pages/ProfilePage.tsx`:

```tsx
import React from 'react';
import { useNavigate } from 'react-router-dom';
import { useTranslation } from 'react-i18next';
import { useAuth } from '../contexts/AuthContext';
import './ProfilePage.css';

export const ProfilePage: React.FC = () => {
    const navigate = useNavigate();
    const { t } = useTranslation();
    const { user } = useAuth();
    // Simulate Premium Status for UI Demo
    const [isPremium, setIsPremium] = React.useState(false);

    const handlePurchase = () => {
        if (window.confirm("Simulate Premium Purchase?")) {
            setIsPremium(true);
        }
    };

    return (
        <div className="profile-page">
            <header className="profile-header">
                <h1>👤 {t('profile.title')}</h1>
                <button className="close-button" onClick={() => navigate('/home')} aria-label="Close">
                    ✕
                </button>
            </header>

            <div className="profile-content">
                {/* Unified Account & Membership Section */}
                <section className="profile-section">

                    {/* Account Status Card (Designed like Subscription Buttons) */}
                    <div className={`account-status-card ${isPremium ? 'premium-active' : ''}`}>
                        <div className="sub-info-left">
                            <span className="sub-title">{t('profile.signedInAs')}</span>
                            <span className="sub-desc">{user?.email || t('profile.guestUser')}</span>
                        </div>
                        <div className="sub-price-right">
                            <div className={`status-badge-pill ${isPremium ? 'premium' : 'free'}`}>
                                {isPremium ? t('profile.status.premium') : t('profile.status.free')}
                            </div>
                        </div>
                    </div>

                    {!isPremium ? (
                        <div className="premium-upgrade-area">
                            <p className="upgrade-prompt">{t('profile.upgradePrompt')}</p>
                            <div className="subscription-options">
                                {/* Quarterly Card */}
                                <button className="sub-btn quarterly" onClick={handlePurchase}>
                                    <div className="sub-info-left">
                                        <span className="sub-title">{t('profile.subscription.quarterly.title')}</span>
                                        <span className="sub-desc">{t('profile.subscription.quarterly.desc')}</span>
                                    </div>
                                    <div className="sub-price-right">
                                        <span className="sub-monthly-price">$1.15 <span className="sub-currency">{t('profile.subscription.currency')}</span></span>
                                        <span className="sub-unit">{t('profile.subscription.unit')}</span>
                                    </div>
                                </button>

                                {/* Yearly Card */}
                                <button className="sub-btn yearly" onClick={handlePurchase}>
                                    <span className="sub-badge">{t('profile.subscription.yearly.badge')}</span>
                                    <div className="sub-info-left">
                                        <span className="sub-title">{t('profile.subscription.yearly.title')}</span>
                                        <span className="sub-desc">{t('profile.subscription.yearly.desc')}</span>
                                    </div>
                                    <div className="sub-price-right">
                                        <span className="sub-monthly-price">$1.00 <span className="sub-currency">{t('profile.subscription.currency')}</span></span>
                                        <span className="sub-unit">{t('profile.subscription.unit')}</span>
                                    </div>
                                </button>
                            </div>
                            <p className="cancel-text">{t('profile.cancelPolicy')}</p>
                        </div>
                    ) : (
                        <div className="premium-active-view">
                            <div className="premium-card-gold">
                                <h3>👑 {t('profile.premiumActive.title')}</h3>
                                <p>{t('profile.premiumActive.desc')}</p>
                            </div>
                        </div>
                    )}
                </section>

                {/* Section 3: My Jello Box Link */}
                <section className="profile-section">
                    <button className="jello-box-link" onClick={() => navigate('/jellobox')}>
                        📚 {t('profile.myJelloBox')}
                    </button>
                </section>
            </div>
        </div>
    );
};
```

---

## 2. ProfilePage.css

Create `src/pages/ProfilePage.css`:

```css
/* Match EncyclopediaPage.css styling */
.profile-page {
    display: flex;
    flex-direction: column;
    height: 100vh;
    height: 100dvh;
    background: linear-gradient(180deg, #87CEEB 0%, #E0F6FF 100%);
    overflow: hidden;
    position: relative;
    font-family: inherit;
}

.profile-header {
    display: flex;
    align-items: center;
    justify-content: space-between;
    padding: 0.7rem 1.5rem;
    background: linear-gradient(180deg, #5a9c7d 0%, #4a8c6d 100%);
    border-bottom: 5px solid #3a6c4d;
    box-shadow: 0 4px 0 rgba(0, 0, 0, 0.2);
    z-index: 10;
    flex-shrink: 0;
    color: white;
}

.profile-header h1 {
    font-size: 1.5rem;
    font-weight: 800;
    margin: 0;
    text-shadow: 2px 2px 0 rgba(0, 0, 0, 0.2);
    display: flex;
    align-items: center;
    gap: 10px;
}

.close-button {
    background: linear-gradient(180deg, #7d5e4f 0%, #6d4e3f 100%);
    border: 3px solid #4d3e2f;
    border-radius: 12px;
    padding: 8px 12px;
    color: white;
    font-weight: 800;
    cursor: pointer;
    box-shadow: 0 4px 0 #3d2e1f;
    transition: all 0.1s;
    font-size: 1.2rem;
    display: flex;
    align-items: center;
    justify-content: center;
    line-height: 1;
}

.close-button:hover {
    filter: brightness(1.1);
    transform: translateY(-2px);
}

.close-button:active {
    transform: translateY(4px);
    box-shadow: 0 0 0 #3d2e1f;
}

/* Content Area */
.profile-content {
    flex: 1;
    overflow-y: auto;
    padding: 20px;
    display: flex;
    flex-direction: column;
    gap: 2rem;
    max-width: 600px;
    margin: 0 auto;
    width: 100%;
    box-sizing: border-box;
}

/* Section Card Style */
.profile-section {
    background: rgba(255, 255, 255, 0.9);
    border-radius: 20px;
    padding: 1.5rem 1.5rem 0.8rem 1.5rem;
    box-shadow: 0 8px 16px rgba(0, 0, 0, 0.1);
    border: 3px solid #fff;
    display: flex;
    flex-direction: column;
    gap: 1rem;
}

.profile-section h2 {
    font-size: 1.2rem;
    color: #3a6c4d;
    margin: 0 0 0.5rem 0;
    border-bottom: 2px dashed #9bc4ad;
    padding-bottom: 0.5rem;
}

/* Account Status Card */
.account-status-card {
    display: flex;
    justify-content: space-between;
    align-items: center;
    padding: 1rem 1.5rem;
    background: #1E2530;
    border-radius: 20px;
    box-shadow: 0 6px 0 #111827;
    color: white;
    margin-bottom: 0.5rem;
    position: relative;
    overflow: hidden;
}

/* Premium Active State (Gold Theme) */
.account-status-card.premium-active {
    background: linear-gradient(135deg, #FFD700 0%, #FFA500 100%);
    box-shadow: 0 6px 0 #B8860B;
    color: #854d0e;
}

.account-status-card.premium-active .sub-title {
    color: #854d0e;
}

.account-status-card.premium-active .sub-desc {
    color: #92400e;
    font-weight: 700;
}

/* Status Badge Pill */
.status-badge-pill {
    padding: 6px 16px;
    border-radius: 12px;
    font-size: 0.9rem;
    font-weight: 800;
    text-transform: uppercase;
    letter-spacing: 1px;
}

.status-badge-pill.free {
    background: #374151;
    color: #9CA3AF;
    border: 2px solid #4B5563;
}

.status-badge-pill.premium {
    background: #fff;
    color: #DAA520;
    box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);
}

/* Upgrade Prompt */
.upgrade-prompt {
    font-size: 0.95rem;
    color: #3a6c4d;
    font-weight: 700;
    text-align: center;
    margin-top: 0;
    margin-bottom: 0.8rem;
}

/* Cancel Policy Text */
.cancel-text {
    font-size: 0.7rem;
    color: #9CA3AF;
    text-align: center;
    margin-top: 1rem;
    margin-bottom: 0;
    font-weight: 500;
}

/* Premium Active View */
.premium-active-view {
    animation: fadeIn 0.5s ease-out;
}

.premium-card-gold {
    background: rgba(255, 255, 255, 0.5);
    border: 3px dashed #DAA520;
    border-radius: 16px;
    padding: 2rem;
    text-align: center;
    color: #8B4513;
    margin-top: 1rem;
}

.premium-card-gold h3 {
    margin: 0 0 0.5rem 0;
    font-size: 1.4rem;
}

@keyframes fadeIn {
    from { opacity: 0; transform: translateY(10px); }
    to { opacity: 1; transform: translateY(0); }
}

/* Subscription Buttons */
.subscription-options {
    display: flex;
    flex-direction: column;
    gap: 1rem;
}

.sub-btn {
    display: flex;
    flex-direction: row;
    justify-content: space-between;
    align-items: center;
    padding: 1.5rem;
    border-radius: 20px;
    border: none;
    cursor: pointer;
    transition: all 0.1s;
    text-align: left;
    position: relative;
    overflow: hidden;
    background: #1E2530;
    color: white;
    box-shadow: 0 6px 0 #111827;
}

.sub-btn:hover {
    transform: translateY(-2px);
    background: #252e3d;
}

.sub-btn:active {
    transform: translateY(4px);
    box-shadow: 0 0 0 transparent !important;
}

.sub-btn.quarterly {
    box-shadow: 0 6px 0 #111827;
    color: white;
}

.sub-btn.yearly {
    background: linear-gradient(135deg, #4ADE80 0%, #2DD4BF 100%);
    box-shadow: 0 6px 0 #059669;
    color: white;
}

.sub-btn.yearly .sub-title,
.sub-btn.yearly .sub-monthly-price,
.sub-btn.yearly .sub-currency {
    color: white;
    text-shadow: 1px 1px 0 rgba(0, 0, 0, 0.1);
}

.sub-btn.yearly .sub-desc,
.sub-btn.yearly .sub-total,
.sub-btn.yearly .sub-unit {
    color: rgba(255, 255, 255, 0.9);
}

.sub-btn.yearly .sub-badge {
    background: #FFD700;
    color: #854d0e;
    box-shadow: 2px 2px 0 rgba(0, 0, 0, 0.1);
}

/* Internal Layout */
.sub-info-left {
    display: flex;
    flex-direction: column;
    gap: 4px;
}

.sub-price-right {
    display: flex;
    flex-direction: column;
    align-items: flex-end;
    gap: 2px;
}

/* Typography */
.sub-title {
    font-size: 1.1rem;
    font-weight: 800;
    color: #FFFFFF;
}

.sub-desc {
    font-size: 0.85rem;
    color: #9CA3AF;
    font-weight: 500;
}

.sub-monthly-price {
    font-size: 1.8rem;
    font-weight: 800;
    line-height: 1;
    color: #4ADE80;
}

.sub-currency {
    font-size: 0.9rem;
    color: #FFFFFF;
    font-weight: 700;
    margin-left: 4px;
}

.sub-unit {
    font-size: 0.8rem;
    color: #9CA3AF;
    font-weight: 600;
}

/* Badge */
.sub-badge {
    position: absolute;
    top: 0;
    left: 0;
    background: #FFD700;
    color: #854d0e;
    font-size: 0.7rem;
    font-weight: 800;
    padding: 4px 12px;
    border-bottom-right-radius: 12px;
    text-transform: uppercase;
    box-shadow: 2px 2px 0 rgba(0, 0, 0, 0.1);
}

/* Jello Box Link */
.jello-box-link {
    background: linear-gradient(135deg, #FFD700 0%, #FF8C00 100%);
    color: white;
    font-weight: 800;
    font-size: 1.2rem;
    padding: 1.5rem;
    border-radius: 20px;
    border: none;
    box-shadow: 0 6px 0 #B8860B;
    cursor: pointer;
    transition: all 0.1s;
    display: flex;
    align-items: center;
    justify-content: center;
    gap: 0.5rem;
    text-shadow: 1px 1px 0 rgba(0, 0, 0, 0.2);
}

.jello-box-link:hover {
    transform: translateY(-2px);
    filter: brightness(1.1);
}

.jello-box-link:active {
    transform: translateY(4px);
    box-shadow: 0 0 0 #B8860B;
}

@media (max-width: 768px) {
    .profile-content {
        padding: 15px;
        gap: 1.5rem;
    }
}
```

---

## 3. Translation Keys (en.ts)

Add to `src/i18n/locales/en.ts` (inside the main export object):

```typescript
profile: {
  title: 'My Profile',
  home: 'Home',
  signedInAs: 'Signed in as',
  guestUser: 'Guest User',
  status: {
    premium: 'PREMIUM',
    free: 'FREE',
    premiumLabel: '✨ Premium Member',
    freeLabel: '🌱 Free Plan',
  },
  upgradePrompt: 'Upgrade to Premium',
  subscription: {
    quarterly: {
      title: 'Billed quarterly',
      desc: 'Now $3.45',
    },
    yearly: {
      title: 'Billed annually',
      desc: 'Total $12.00',
      badge: 'Best Value',
    },
    unit: '/ month',
    currency: 'USD',
  },
  cancelPolicy: 'All plans can be cancelled anytime.',
  premiumActive: {
    title: 'You are a Premium Member!',
    desc: 'Thank you for your support.',
  },
  myJelloBox: 'My Jello Box',
},
```

---

## 4. Route Configuration (App.tsx)

Add to `src/App.tsx`:

### Import
```tsx
import { ProfilePage } from './pages/ProfilePage';
```

### Route (inside `<Routes>`)
```tsx
<Route path="/profile" element={
  <ProtectedRoute>
    <ProfilePage />
  </ProtectedRoute>
} />
```

---

## 5. Navigation Trigger (Optional)

To navigate to the Profile page, use:

```tsx
import { useNavigate } from 'react-router-dom';

const navigate = useNavigate();

// Example: Avatar click handler
<button onClick={() => navigate('/profile')}>Go to Profile</button>
```

---

## Dependencies

Make sure the following are installed:
- `react-router-dom`
- `react-i18next`
- `AuthContext` (for `useAuth` hook)

---

## Notes

1. **Premium Integration**: Currently uses local state (`isPremium`) for demo. Replace `handlePurchase` with actual payment API integration.
2. **JelloBox Route**: Expects `/jellobox` route to exist. Update if your collection page uses a different path.
3. **Header Navigation**: The close button navigates to `/home`. Adjust as needed.
