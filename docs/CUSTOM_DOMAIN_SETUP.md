...
- Cloudflare zone 활성화 및 DNS 관리 전환 완료
...
- API 경로는 `api.grogrojello.com` 단일 기준으로 유지
...
- `api.grogrojello.com` 접근 실패 시에는 fallback이 아니라 네트워크 환경을 직접 점검
...
- 보조 URL: `https://www.grogrojello.com`
...
- 대표 웹 주소는 `https://grogrojello.com`
- `https://www.grogrojello.com` 은 Cloudflare Redirect Rule로 `https://grogrojello.com` 으로 영구 리다이렉트
- `grogrojello.com` apex는 `DNS only`로 Vercel 직결
- `www`는 `Proxied` 상태에서 Redirect Rule 적용
- `api.grogrojello.com` 은 Cloudflare Worker Custom Domain
...
- API는 `https://api.grogrojello.com`
...
- `www`는 Cloudflare Redirect Rule로 `grogrojello.com` 으로 전달
- `Worker api.grogrojello.com` : `Proxied`
...
대표 도메인 `grogrojello.com` 자체의 라우팅은 추가 수정 가능성이 낮습니다.
...
- 브라우저/리졸버 환경에 따라 `api.grogrojello.com` 자체 접근이 실패하면 클라우드 복원도 실패할 수 있으므로 네트워크 환경을 먼저 점검해야 합니다.
...
### A. Search Console / Analytics / 광고 픽셀 도메인 업데이트
...
Apple 로그인을 실제로 운영할 계획이라면 `grogrojello.com` 기준으로 별도 검증이 필요합니다.

### C. 검색 공개 전환

현재는 검색 차단이 적용돼 있으므로 오픈 시 아래를 해제해야 합니다.

- [`public/robots.txt`](/Users/somniavisk/Desktop/puzzleletic/public/robots.txt)
- [`index.html`](/Users/somniavisk/Desktop/puzzleletic/index.html)
- [`vercel.json`](/Users/somniavisk/Desktop/puzzleletic/vercel.json)
...
- DNS 전파 완료
- SSL 인증서 발급 완료
- Cloudflare zone 활성화 완료
- Cloudflare Redirect Rule 활성화 완료
...
- `www.grogrojello.com` 접속 시 `grogrojello.com` 으로 즉시 이동 확인

### 네트워크 이슈 점검 메모

- 특정 Safari / 특정 Wi‑Fi 조합에서는 `https://api.grogrojello.com/api/users/test` 자체가 열리지 않으며, 이 경우 클라우드 복원 실패로 기본 상태처럼 보일 수 있습니다.
- 이 증상은 운영 D1 데이터 유실과는 별개일 수 있으므로, premium/캐릭터 소실로 보일 때는 먼저 아래를 확인합니다.
  - `https://api.grogrojello.com/api/users/test` 가 `{"error":"Missing or invalid Authorization header"}` 를 반환하는지
  - 다른 Wi‑Fi 또는 다른 브라우저에서 같은 계정이 정상 복원되는지
- 실제 조사 결과 `test34@gmail.com` 계정은 운영 D1에 premium과 기존 `game_data`가 정상 보존되어 있었고, 특정 Wi‑Fi/Safari 환경에서만 복원 실패가 재현됐습니다.
- 따라서 이 경우는 앱 데이터 유실보다 네트워크/DNS 환경 문제로 먼저 판단해야 합니다.

### 하이브리드 저장 보호 로직 메모

- `notFound` 보호 로직은 유지하되, 현재 사용자 로컬 캐시에 의미 있는 데이터가 있으면 계정 생성 시각과 무관하게 그 로컬 데이터를 기준으로 cloud row 복구를 시도하도록 완화했습니다.
- 여기서 의미 있는 로컬 데이터란 `hasCharacter`, XP, GRO, Stars, history, gameScores 같은 실제 플레이 흔적이 있는 상태를 말합니다.
- 이 조정은 기존 하이브리드 저장 흐름을 바꾸지 않기 위해 `saveToCloud`, 자동 저장, 일반 `syncUserData` 흐름은 건드리지 않고 `notFound` 분기 판단만 수정한 것입니다.
- 따라서 “로컬에는 현재 사용자 데이터가 있는데 cloud row가 없다”는 상황에서만 예외적으로 복구를 허용하고, 로컬도 비어 있는 오래된 계정에 대해서는 기존 보호를 유지합니다.

### 로컬 모바일 테스트 메모

- 모바일에서 로컬 Vite 서버(`http://172.x.x.x:5174`, `http://192.168.x.x:5174`)로 접속해 운영 Worker API를 테스트할 수 있도록 Worker CORS에 사설 IP 대역 개발 origin을 허용했습니다.
- 관련 반영 후 Cloudflare Worker를 재배포했습니다.
- 현재 Worker API는 `https://api.grogrojello.com` custom domain 기준으로 사용합니다.
...
1. Vercel 커스텀 도메인 연결
2. Cloudflare DNS 연결 및 zone 활성화
3. Firebase Authorized Domains 추가
4. 실제 로그인 재테스트
5. `www -> apex` canonical redirect 확정
...
2. Namecheap -> Cloudflare 네임서버 전환 완료
3. Firebase Authorized Domains 추가 완료
4. 이메일 로그인 검증 완료
5. Google 로그인 검증 완료
6. Worker CORS 제한 및 재배포 완료
7. `api.grogrojello.com` Custom Domain 연결 완료
8. `www.grogrojello.com -> grogrojello.com` Redirect Rule 완료

현재 남은 후속 개선 후보:

1. 필요 시 [`backend/api-grogrojello/src/index.js`](/Users/somniavisk/Desktop/puzzleletic/backend/api-grogrojello/src/index.js) 의 허용 Origin을 환경변수 기반으로 관리
2. Apple 로그인 운영 검증
3. 오픈 시 검색 차단 해제 및 SEO 공개 전환
