...
type AnimationState = 'idle' | 'slicing' | 'shaking';
...
const FRUITS: Fruit[] = [
  { emoji: '🍉', color: '#4ADE80' }, // lime-400
  { emoji: '🍓', color: '#F87171' }, // red-400
  { emoji: '🍊', color: '#FB923C' }, // orange-400
  { emoji: '🍋', color: '#FACC15' }, // yellow-400
  { emoji: '🥝', color: '#A3E635' }, // lime-400
  { emoji: '🍎', color: '#EF4444' }, // red-500
  { emoji: '🍍', color: '#FBBF24' }, // amber-400
];
...
const generateProblem = (difficulty: number): Problem => {
  let fruitNumber: number, solution: number;
  const operator = '-';

  if (difficulty === 1) { // Beginner: Single-digit - Single-digit
    fruitNumber = Math.floor(Math.random() * 8) + 2; // 2 to 9
    solution = Math.floor(Math.random() * (fruitNumber - 1)) + 1; // 1 to fruitNumber-1
  } else if (difficulty === 2) { // Intermediate: Double-digit (10-19) - Single-digit
    fruitNumber = Math.floor(Math.random() * 10) + 10; // 10 to 19
    solution = Math.floor(Math.random() * 9) + 1;      // 1 to 9
  } else { // Advanced: Double-digit (11-20) - Double-digit (10-19)
    fruitNumber = Math.floor(Math.random() * 10) + 11; // 11 to 20
    solution = Math.floor(Math.random() * (fruitNumber - 10)) + 10; // Guarantees solution is 10 to (fruitNumber - 1)
  }

  const resultNumber = fruitNumber - solution;

  const options: number[] = [solution];
  while (options.length < 4) {
    const range = Math.max(5, Math.ceil(solution / 2));
    const wrongOption = solution + (Math.floor(Math.random() * (range * 2)) - range);
    if (wrongOption > 0 && !options.includes(wrongOption) && wrongOption !== solution) {
  ...
  return { id: Math.random(), fruitNumber, operator, resultNumber, solution, options, difficulty, fruit };
};
...
const App = () => {
  ...
  useEffect(() => {
    ...
    audioRef.current = new Audio('https://soundimage.org/wp-content/uploads/2017/05/Hypnotic-Puzzle3.mp3');
  ...
  const adjustDifficulty = useCallback((isCorrect: boolean) => {
    if (isCorrect) {
        ...
        // Difficulty increases after 8 correct answers at the current level.
        if (difficultyLevel < 3 && newCorrectAtCurrentDifficulty >= 8) {
  ...
  }, [correctAnswersAtCurrentDifficulty, difficultyLevel, consecutiveWrong]);
  ...
  const generateParticles = (type: 'correct' | 'wrong', count = 10, emojiOverride?: string) => {
    ...
    for (let i = 0; i < count; i++) {
      newParticles.push({
        id: Math.random(),
        emoji: emojiOverride || (type === 'correct' ? ['🎉', '✨', '🍉', '🍓', '🌟'][Math.floor(Math.random() * 5)] : ['💥', '😵', '❌'][Math.floor(Math.random() * 3)]),
  ...
  const handleDrop = (event: React.DragEvent<HTMLDivElement>) => {
      ...
      if (gameState !== 'playing' || animationState !== 'idle') return;

      const choice = Number(event.dataTransfer.getData("text/plain"));
      if (!choice || !currentProblem) return;

      const isCorrect = choice === currentProblem.solution;
      
      setAnimationState(isCorrect ? 'slicing' : 'shaking');
  ...
  const handleDragStart = (event: React.DragEvent<HTMLButtonElement>, choice: number) => {
  ...
  const setupNewGame = useCallback(() => {
    ...
    setCurrentProblem(problemSet[Math.floor(Math.random() * problemSet.length)] as any);
  }, []);
  ...
  const handleDownloadImage = useCallback(() => {
    ...
    toPng(gameOverCardRef.current, { 
      ...
      .then((dataUrl) => {
        ...
        link.download = 'fruit-slash-result.png';
        link.href = dataUrl;
        link.click();
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-400 via-pink-500 to-red-500 p-4 relative overflow-hidden font-sans flex flex-col">
      ...
      <main className="flex-grow">
          ...
          <div className="max-w-md mx-auto">
            ...
            <div className="bg-white rounded-2xl shadow-2xl p-4 md:p-6 mb-3" style={{ background: '#FEF9E6' }}>
              <div className="text-center relative">
                ...
                {currentProblem ? (
                  <div className="flex flex-col items-center w-full max-w-xs mx-auto">
                     <div className="bg-yellow-100 border-2 border-dashed border-orange-300 rounded-lg px-4 py-2 mb-4 shadow-inner">
                        <div className="text-3xl font-bold text-gray-700 tracking-wider flex items-center gap-x-3">
                            <span>{currentProblem.fruitNumber}</span>
                            <span className="text-orange-500">{currentProblem.operator}</span>
                            <span className="text-orange-500">?</span>
                            <span>=</span>
                            <span>{currentProblem.resultNumber}</span>
                        </div>
                    </div>
                    ...
                    <div 
                        ...
                        className={`relative w-40 h-40 mb-4 rounded-full transition-all duration-300 ${isDraggingOver ? 'scale-110 shadow-lg' : ''} ${animationState === 'shaking' ? 'animate-shake' : ''}`}
                    >
                      {animationState === 'slicing' ? (
                        <>
                            <div className="absolute w-full h-full animate-slice-left overflow-hidden">
                                <div className="relative w-full h-full rounded-full flex items-center justify-center" style={{backgroundColor: currentProblem.fruit.color, clipPath: 'polygon(0% 0%, 50% 0%, 50% 100%, 0% 100%)'}}>
                                    <span className="absolute text-8xl opacity-30 select-none">{currentProblem.fruit.emoji}</span>
                                    <span className="relative text-6xl font-bold text-white" style={{ textShadow: '2px 2px 4px rgba(0,0,0,0.5)' }}>{currentProblem.fruitNumber}</span>
                                </div>
                            </div>
                            <div className="absolute w-full h-full animate-slice-right overflow-hidden">
                               <div className="relative w-full h-full rounded-full flex items-center justify-center" style={{backgroundColor: currentProblem.fruit.color, clipPath: 'polygon(50% 0%, 100% 0%, 100% 100%, 50% 100%)'}}>
                                    <span className="absolute text-8xl opacity-30 select-none -ml-px">{currentProblem.fruit.emoji}</span>
                                    <span className="relative text-6xl font-bold text-white -ml-px" style={{ textShadow: '2px 2px 4px rgba(0,0,0,0.5)' }}>{currentProblem.fruitNumber}</span>
                                </div>
                            </div>
                        </>
                      ) : (
                          <div className="relative w-full h-full rounded-full flex items-center justify-center shadow-lg" style={{ backgroundColor: currentProblem.fruit.color }}>
                            <span className="absolute text-8xl opacity-30 select-none">{currentProblem.fruit.emoji}</span>
                            <span className="relative text-6xl font-bold text-white" style={{ textShadow: '2px 2px 4px rgba(0,0,0,0.5)' }}>{currentProblem.fruitNumber}</span>
                          </div>
                      )}
                     ...
                     <div className="w-full">
                        <p className="text-center font-semibold text-gray-600 mb-2">{t('question')}</p>
                        <div className="grid grid-cols-2 gap-3">
                            {currentProblem.options.map((option) => (
                                <button
                                    key={option}
                                    draggable={gameState === 'playing' && animationState === 'idle'}
                                    onDragStart={(e) => handleDragStart(e, option)}
                                    disabled={gameState !== 'playing' || animationState !== 'idle'}
                                    className={`h-16 text-3xl font-bold rounded-lg transition-all transform active:scale-95 shadow-md flex items-center justify-center gap-2 text-gray-800 disabled:cursor-not-allowed disabled:bg-gray-300 relative overflow-hidden group bg-gray-200 ${gameState === 'playing' && animationState === 'idle' ? 'hover:bg-gray-300 cursor-grab active:cursor-grabbing' : ''}`}
                                >
                                    <span>{option}</span>
                                    <span role="img" aria-label="knife" className="text-2xl text-gray-500 group-hover:text-gray-700 transition-colors">🔪</span>
                                </button>
                            ))}
                        </div>
                    </div>
                ...
                <div className="h-16 mt-4">
                   {showHint ? (
                    <div className={`border-l-4 p-2 rounded mt-2 text-sm text-left bg-orange-50 border-orange-400 text-orange-800`}>
                      <p><strong>{t('hintLabel')}</strong> {currentProblem ? t('hintText_parity', { parity: currentProblem.solution % 2 === 0 ? t('hint_even') : t('hint_odd') }) : ''} {hintUsed && t('hintUsedText')}</p>
                    </div>
                  ) : feedback ? (
