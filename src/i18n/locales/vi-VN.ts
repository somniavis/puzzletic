// Global English Locales
// Updated for Compare Critters
import fishingCountVi from '../../games/math/adventure/level1/FishingCount/locales/vi-VN';
import tenFrameNumberLocale from '../../games/math/adventure/level1/TenFrameNumber/locales/vi-VN';
import tenFrameAddLocale from '../../games/math/adventure/level1/TenFrameAdd/locales/vi-VN';
import moleWhackLocale from '../../games/math/adventure/level1/MoleWhack/locales/vi-VN';
import roundCountingVi from '../../games/math/adventure/level1/RoundCounting/locales/vi-VN';
import numberHiveVi from '../../games/math/adventure/level1/NumberHive/locales/vi-VN';
import fruitSliceVi from '../../games/math/adventure/level1/FruitSlice/locales/vi-VN';
import numberBalanceVi from '../../games/math/adventure/level1/NumberBalance/locales/vi-VN';
import mathArcheryVi from '../../games/math/adventure/level1/MathArchery/locales/vi-VN';
import jelloFeedingVi from '../../games/math/adventure/level1/JelloFeeding/locales/vi-VN';
import frontAdditionVi from '../../games/math/genius/FrontAddition/locales/vi-VN';
import frontSubtractionVi from '../../games/math/genius/FrontSubtraction/locales/vi-VN';
import backMultiplicationVi from '../../games/math/genius/BackMultiplication/locales/vi-VN';
import tenFrameCountVi from '../../games/math/adventure/level2/TenFrameCount/locales/vi-VN';
import mathPinwheelVi from '../../games/math/adventure/level2/PinwheelPop/locales/vi-VN';
import compareCrittersVi from '../../games/math/adventure/level1/CompareCritters/locales/vi-VN';
import deepSeaDiveVi from '../../games/math/adventure/level2/DeepSeaDive/locales/vi-VN';
import signalHunterVi from '../../games/brain/level2/SignalHunter/locales/vi-VN';
import lockOpeningLocale from '../../games/math/adventure/level2/LockOpening/locales/vi-VN';
import magicPotionLocale from '../../games/math/adventure/level2/MagicPotion/locales/vi-VN';

import colorLinkVi from '../../games/brain/level1/ColorLink/locales/vi-VN';
import pairUpTwinVi from '../../games/brain/level1/PairUpTwin/locales/vi-VN';
import wildLinkVi from '../../games/brain/level2/WildLink/locales/vi-VN';
import pairUpConnectVi from '../../games/brain/level2/PairUpConnect/locales/vi-VN';
import animalBanquetVi from '../../games/brain/level2/AnimalBanquet/locales/vi-VN';
import colorMixVi from '../../games/brain/level2/ColorMix/locales/vi-VN';
import ufoInvasionVi from '../../games/math/adventure/level2/UFOInvasion/locales/vi-VN';
import cargoTrainVi from '../../games/math/adventure/level2/CargoTrain/locales/vi-VN.ts';
import rocketLauncherVi from '../../games/math/adventure/level2/RocketLauncher/locales/vi-VN';
import shapeSumLinkVi from '../../games/math/adventure/level2/ShapeSumLink/locales/vi-VN';
import fruitBoxVi from '../../games/math/adventure/level3/FruitBox/locales/vi-VN';
import iceStackingVi from '../../games/math/adventure/level3/IceStacking/locales/vi-VN';
import floorTilerVi from '../../games/math/adventure/level3/FloorTiler/locales/vi-VN';
import frogJumpVi from '../../games/math/adventure/level3/FrogJump/locales/vi-VN';
import chipCashierVi from '../../games/math/adventure/level3/ChipCashier/locales/vi-VN';
import cellCloneVi from '../../games/math/adventure/level3/CellClone/locales/vi-VN';
import fairShareLocale from "../../games/math/adventure/level3/FairShare/locales/vi-VN";
import donutShopLocale from "../../games/math/adventure/level3/DonutShop/locales/vi-VN";
import chocolateSplitLocale from "../../games/math/adventure/level3/ChocolateSplit/locales/vi-VN";
import scorpionKingLocale from "../../games/math/adventure/level3/ScorpionKing/locales/vi-VN";
import pizzaPizzaLocale from "../../games/math/adventure/level3/PizzaPizza/locales/vi-VN";
import riverCrossingLocale from "../../games/math/adventure/level3/RiverCrossing/locales/vi-VN";
import neonMatrixVi from '../../games/math/adventure/level3/NeonMatrix/locales/vi-VN';
import threeLeafCloverLocale from '../../games/math/adventure/level3/ThreeLeafClover/locales/vi-VN';
import signShifterLocale from '../../games/math/adventure/level3/SignShifter/locales/vi-VN';
import hexHiveSixLocale from '../../games/math/adventure/level3/HexHiveSix/locales/vi-VN';
import flightCalendarLocale from '../../games/math/adventure/level3/FlightCalendar/locales/vi-VN';
import tenFramePopLocale from '../../games/math/adventure/level3/TenFramePop/locales/vi-VN';
import beginnerWizardVi from '../../games/math/adventure/level3/BeginnerWizard/locales/vi-VN';
import constellationFinderVi from '../../games/math/adventure/level3/ConstellationFinder/locales/vi-VN';
import trollAttackVi from '../../games/math/adventure/level3/TrollAttack/locales/vi-VN';
import blockTowerVi from '../../games/brain/level2/BlockTower/locales/vi-VN';
import sharpshooterVi from '../../games/brain/level2/Sharpshooter/locales/vi-VN';
import mazeEscapeVi from '../../games/brain/level1/MazeEscape/locales/vi-VN';
import mazeHunterVi from '../../games/brain/level2/MazeHunter/locales/vi-VN';
import ticTacToeVi from '../../games/brain/level3/TicTacToe/locales/vi-VN';
import omokVi from '../../games/brain/level3/Omok/locales/vi-VN';

const emotionEmojiVi = {
  eat: {
    l1: { affectionate: ['😋', '🍓', '🍭'], playful: ['😋', '🍔', '😛'], calm: ['🥄', '🍵', '🍚'], shy: ['🐹', '🍪', '😋'], grumpy: ['😐', '🦴', '😑'], energetic: ['🍡', '🍚', '🥕'] },
    l2: { affectionate: ['😋', '🍓', '🍭'], playful: ['😋', '🍔', '😛'], calm: ['🥄', '🍵', '🍚'], shy: ['🐹', '🍪', '😋'], grumpy: ['😐', '🦴', '😑'], energetic: ['🍡', '🍚', '🥕'] },
    l3: { affectionate: ['😋', '🍓', '🍭'], playful: ['😋', '🍔', '😛'], calm: ['🥄', '🍵', '🍚'], shy: ['🐹', '🍪', '😋'], grumpy: ['😐', '🦴', '😑'], energetic: ['🍡', '🍚', '🥕'] },
  },
  eat_aftereffect: {
    l1: { affectionate: ['😳', '💩', '🤭'], playful: ['😳', '💩', '😯'], calm: ['😶‍🌫️', '😳', '🤭'], shy: ['🤭', '😳', '🫣'], grumpy: ['😮‍💨', '😑', '💩'], energetic: ['😯', '💨', '💩'] },
    l2: { affectionate: ['😳', '💩', '🤭'], playful: ['😳', '💩', '😯'], calm: ['😶‍🌫️', '😳', '🤭'], shy: ['🤭', '😳', '🫣'], grumpy: ['😮‍💨', '😑', '💩'], energetic: ['😯', '💨', '💩'] },
    l3: { affectionate: ['😳', '💩', '🤭'], playful: ['😳', '💩', '😯'], calm: ['😶‍🌫️', '😳', '🤭'], shy: ['🤭', '😳', '🫣'], grumpy: ['😮‍💨', '😑', '💩'], energetic: ['😯', '💨', '💩'] },
  },
  medicine_pill: {
    l1: { affectionate: ['💊', '😣', '🤒'], playful: ['💊', '😬', '🥴'], calm: ['💊', '😮‍💨', '😶'], shy: ['😣', '🫣', '💊'], grumpy: ['😒', '💊', '😤'], energetic: ['💊', '😬', '✨'] },
    l2: { affectionate: ['💊', '😣', '🤒'], playful: ['💊', '😬', '🥴'], calm: ['💊', '😮‍💨', '😶'], shy: ['😣', '🫣', '💊'], grumpy: ['😒', '💊', '😤'], energetic: ['💊', '😬', '✨'] },
    l3: { affectionate: ['💊', '😣', '🤒'], playful: ['💊', '😬', '🥴'], calm: ['💊', '😮‍💨', '😶'], shy: ['😣', '🫣', '💊'], grumpy: ['😒', '💊', '😤'], energetic: ['💊', '😬', '✨'] },
  },
  medicine_shot: {
    l1: { affectionate: ['💉', '😳', '😖'], playful: ['💉', '😱', '😬'], calm: ['💉', '😮‍💨', '😶'], shy: ['🫣', '😖', '💉'], grumpy: ['😤', '💉', '😒'], energetic: ['💉', '😤', '✨'] },
    l2: { affectionate: ['💉', '😳', '😖'], playful: ['💉', '😱', '😬'], calm: ['💉', '😮‍💨', '😶'], shy: ['🫣', '😖', '💉'], grumpy: ['😤', '💉', '😒'], energetic: ['💉', '😤', '✨'] },
    l3: { affectionate: ['💉', '😳', '😖'], playful: ['💉', '😱', '😬'], calm: ['💉', '😮‍💨', '😶'], shy: ['🫣', '😖', '💉'], grumpy: ['😤', '💉', '😒'], energetic: ['💉', '😤', '✨'] },
  },
  clean_spot: {
    l1: { affectionate: ['🧹', '✨', '😌'], playful: ['🧹', '🗞️', '✨'], calm: ['✨', '😌', '🫧'], shy: ['😌', '✨', '🫧'], grumpy: ['🧹', '😑', '✨'], energetic: ['✨', '🌟', '🧹'] },
    l2: { affectionate: ['🧹', '✨', '😌'], playful: ['🧹', '🗞️', '✨'], calm: ['✨', '😌', '🫧'], shy: ['😌', '✨', '🫧'], grumpy: ['🧹', '😑', '✨'], energetic: ['✨', '🌟', '🧹'] },
    l3: { affectionate: ['🧹', '✨', '😌'], playful: ['🧹', '🗞️', '✨'], calm: ['✨', '😌', '🫧'], shy: ['😌', '✨', '🫧'], grumpy: ['🧹', '😑', '✨'], energetic: ['✨', '🌟', '🧹'] },
  },
  clean_fresh: {
    l1: { affectionate: ['🚿', '✨', '🌿'], playful: ['🪥', '✨', '🫧'], calm: ['🌿', '😌', '✨'], shy: ['🫧', '✨', '🌸'], grumpy: ['🪥', '😑', '✨'], energetic: ['🚿', '💫', '✨'] },
    l2: { affectionate: ['🚿', '✨', '🌿'], playful: ['🪥', '✨', '🫧'], calm: ['🌿', '😌', '✨'], shy: ['🫧', '✨', '🌸'], grumpy: ['🪥', '😑', '✨'], energetic: ['🚿', '💫', '✨'] },
    l3: { affectionate: ['🚿', '✨', '🌿'], playful: ['🪥', '✨', '🫧'], calm: ['🌿', '😌', '✨'], shy: ['🫧', '✨', '🌸'], grumpy: ['🪥', '😑', '✨'], energetic: ['🚿', '💫', '✨'] },
  },
  joy: {
    l1: { affectionate: ['😊', '😄', '🙌'], playful: ['😆', '🎶', '😁'], calm: ['😌', '🎵', '👌'], shy: ['☺️', '😳', '😚'], grumpy: ['😒', '😏', '🙄'], energetic: ['✨', '👍', '😲'] },
    l2: { affectionate: ['🥰', '🌈', '🎈'], playful: ['🏃', '⚽️', '🦖'], calm: ['☕️', '🍀', '🍃'], shy: ['😽', '🌸', '🙈'], grumpy: ['😑', '😼', '😮'], energetic: ['🏆', '💪', '🚀'] },
    l3: { affectionate: ['❤️', '💖', '🌟'], playful: ['💡', '👑', '😂'], calm: ['🙏', '🙇', '👏'], shy: ['🍅', '🎁', '🦄'], grumpy: ['🆗', '💬', '😤'], energetic: ['🔥', '🌍', '📢'] },
  },
  love: {
    l1: { affectionate: ['😘', '🐣', '👋'], playful: ['🎣', '💎', '👉'], calm: ['🌤️', '👤', '🛡️'], shy: ['😺', '🧣', '😊'], grumpy: ['😕', '✋', '😤'], energetic: ['💘', '🤜🤛', '👍'] },
    l2: { affectionate: ['🤗', '🌞', '💋'], playful: ['👐', '🐨', '🏹'], calm: ['💎', '🌲', '🤝'], shy: ['☺️', '💌', '💓'], grumpy: ['😳', '⏱️', '😒'], energetic: ['⚡️', '🤖', '♾️'] },
    l3: { affectionate: ['🏡', '👫', '💝'], playful: ['👮', '🎖️', '🫡'], calm: ['💐', '🧶', '✨'], shy: ['🫂', '😻', '🍀'], grumpy: ['😎', '😠', '🐕'], energetic: ['👯', '🚀', '🎉'] },
  },
  playful: {
    l1: { affectionate: ['😋', '🍓', '🍭'], playful: ['🐊', '🍔', '😛'], calm: ['🥄', '🍵', '🥗'], shy: ['🐹', '🍪', '🐿️'], grumpy: ['😐', '🦴', '😑'], energetic: ['🍡', '🍚', '🥕'] },
    l2: { affectionate: ['🎡', '🤣', '😆'], playful: ['🔫', '⚔️', '🛡️'], calm: ['🧐', '🎲', '😏'], shy: ['🫣', '👉👈', '👻'], grumpy: ['🛑', '😩', '👋'], energetic: ['🐇', '🌀', '💨'] },
    l3: { affectionate: ['🎪', '🎈', '😵'], playful: ['🎆', '🎺', '🦸'], calm: ['🥺', '😲', '👏'], shy: ['🤭', '✨', '🤩'], grumpy: ['🙀', '😼', '👀'], energetic: ['📈', '🚄', '🏎️'] },
  },
  neutral: {
    l1: { affectionate: ['🧸', '🥺', '💧'], playful: ['🤔', '😈', '💬'], calm: ['🤫', '😶', '🕊️'], shy: ['👀', '😐', '🐜'], grumpy: ['😩', '🌬️', '😑'], energetic: ['🤸', '🚪', '💣'] },
    l2: { affectionate: ['🔭', '👣', '🐕'], playful: ['⚡️', '👆', '📢'], calm: ['💭', '🧘', '🍵'], shy: ['👉👈', '🍃', '🔇'], grumpy: ['🌵', '🛌', '😤'], energetic: ['🐜', '💨', '📢'] },
    l3: { affectionate: ['😿', '🍂', '😭'], playful: ['🤪', '😈', '🖌️'], calm: ['🌑', '🤐', '😶'], shy: ['🤧', '😨', '🌃'], grumpy: ['✋', '🙉', '👎'], energetic: ['🌋', '🏃', '😫'] },
  },
  sleepy: {
    l1: { affectionate: ['🥱', '🎶', '🧸'], playful: ['🥴', '🙅', '😪'], calm: ['🫢', '🛀', '🍵'], shy: ['😵', '🛌', '🤤'], grumpy: ['💡', '🚫', '💤'], energetic: ['🚲', '🔋', '😵'] },
    l2: { affectionate: ['🌈', '🐑', '💤'], playful: ['😴', '🤥', '💬'], calm: ['🛌', '🌙', '🤫'], shy: ['🛌', '😪', '🌛'], grumpy: ['👿', '🐕', '✋'], energetic: ['🔌', '📴', '🪫'] },
    l3: { affectionate: ['🌠', '💜', '😴'], playful: ['💤', '🐴', '🛌'], calm: ['🌑', '😶', '🔇'], shy: ['👶', '🌙', '🫧'], grumpy: ['😑', '💢', '💤'], energetic: ['🪫', '☠️', '😵'] },
  },
  sick: {
    l1: { affectionate: ['🤕', '🩹', '🤒'], playful: ['😖', '🤥', '💥'], calm: ['🌡️', '💫', '🛌'], shy: ['😣', '😢', '🤧'], grumpy: ['🤦', '💢', '😔'], energetic: ['😵', '📉', '🥴'] },
    l2: { affectionate: ['😭', '🆘', '💦'], playful: ['🦆', '📢', '🌀'], calm: ['🏥', '🔥', '💊'], shy: ['😓', '😨', '🚑'], grumpy: ['💊', '😤', '😣'], energetic: ['🛌', '🏳️', '📉'] },
    l3: { affectionate: ['😿', '👋', '🛌'], playful: ['😵', '🎮', '❌'], calm: ['📉', '🌫️', '🏥'], shy: ['🧎', '😶', '🏥'], grumpy: ['👿', '🤐', '😫'], energetic: ['⏹️', '⏸️'] },
  },
  worried: {
    l1: { affectionate: ['😟', '😥', '⚠️'], playful: ['😲', '💦', '😳'], calm: ['🔍', '⁉️', '🛡️'], shy: ['😖', '👣', '😢'], grumpy: ['😒', '😠', '☁️'], energetic: ['🚨', '☢️', '🚧'] },
    l2: { affectionate: ['😨', '🛡️', '🥶'], playful: ['🏃', '📢', '😱'], calm: ['✋', '⛔️', '↩️'], shy: ['🫨', '📦', '🙈'], grumpy: ['😫', '😤', '🦁'], energetic: ['🫢', '🆘', '🌪️'] },
    l3: { affectionate: ['😭', '💔', '🧟'], playful: ['😭', '🚑', '🤱'], calm: ['📉', '🧩', '❌'], shy: ['🥀', '💦', '😱'], grumpy: ['🚮', '😫', '🤬'], energetic: ['🤯', '🧱', '🆘'] },
  },
  angry: {
    l1: { affectionate: ['😤', '😞', '😒'], playful: ['👊', '🥊', '⚔️'], calm: ['😔', '✋', '😐'], shy: ['🥺', '😢', '😿'], grumpy: ['👉', '🤫', '🌩️'], energetic: ['😡', '🥋', '🦁'] },
    l2: { affectionate: ['😠', '🙅', '🤥'], playful: ['💣', '🐊', '🦖'], calm: ['⚠️', '🚫', '🛑'], shy: ['😭', '👋', '😣'], grumpy: ['👿', '🙈', '🗯️'], energetic: ['👺', '💢', '🥊'] },
    l3: { affectionate: ['😤', '🙈', '💢'], playful: ['🚂', '🌪️', '😈'], calm: ['😑', '🛑', '🔚'], shy: ['🌊', '😭', '👶'], grumpy: ['🌋', '💨', '😫'], energetic: ['🔥', '🔨', '🤬'] },
  },
};

const moodToneStemsVi: Record<string, Record<string, string>> = {
  joy: {
    affectionate: 'Vui lắm',
    playful: 'Chơi thôi',
    calm: 'Dễ chịu quá',
    shy: 'Ngại quá',
    grumpy: 'Hừm...',
    energetic: 'Tràn năng lượng',
  },
  love: {
    affectionate: 'Thương bạn quá',
    playful: 'Ôm cái nào',
    calm: 'Yên tâm rồi',
    shy: 'Tim đập nhanh quá',
    grumpy: 'Hơi dỗi rồi',
    energetic: 'Yêu thương bùng nổ',
  },
  playful: {
    affectionate: 'Thích quá',
    playful: 'Vui ghê',
    calm: 'Nhẹ nhàng thôi',
    shy: 'Hehe...',
    grumpy: 'Đừng chọc nữa',
    energetic: 'Quẩy nào',
  },
  eat: {
    affectionate: 'Ngon quá',
    playful: 'Nhai nào',
    calm: 'Ngon nè',
    shy: 'Ngon ghê',
    grumpy: 'Cũng được',
    energetic: 'Ăn nữa',
  },
  eat_aftereffect: {
    affectionate: 'Ơ kìa',
    playful: 'Ục ục',
    calm: 'Có tín hiệu',
    shy: 'Ngại quá',
    grumpy: 'Lại nữa à',
    energetic: 'Nhanh lên',
  },
  medicine_pill: {
    affectionate: 'Uống nhé',
    playful: 'Đắng ghê',
    calm: 'Từ từ nào',
    shy: 'Hơi sợ',
    grumpy: 'Ghét thuốc',
    energetic: 'Nuốt nào',
  },
  medicine_shot: {
    affectionate: 'Tiêm à',
    playful: 'Á á',
    calm: 'Một chút thôi',
    shy: 'Sợ quá',
    grumpy: 'Ghét tiêm',
    energetic: 'Cố lên',
  },
  clean_spot: {
    affectionate: 'Sạch rồi',
    playful: 'Quét sạch',
    calm: 'Gọn rồi',
    shy: 'Đỡ rồi',
    grumpy: 'Ổn hơn',
    energetic: 'Xong rồi',
  },
  clean_fresh: {
    affectionate: 'Thơm quá',
    playful: 'Lấp lánh',
    calm: 'Sạch ghê',
    shy: 'Dễ chịu quá',
    grumpy: 'Ổn rồi',
    energetic: 'Tươi mới',
  },
  neutral: {
    affectionate: 'Ở đây với mình nhé',
    playful: 'Làm gì bây giờ',
    calm: 'Cứ từ từ thôi',
    shy: 'Mình hơi ngại',
    grumpy: 'Không thích lắm',
    energetic: 'Muốn làm gì đó',
  },
  sleepy: {
    affectionate: 'Buồn ngủ quá',
    playful: 'Chơi chút nữa thôi',
    calm: 'Mình nghỉ nhé',
    shy: 'Ngáp rồi nè',
    grumpy: 'Đừng làm ồn',
    energetic: 'Hết pin rồi',
  },
  sick: {
    affectionate: 'Mệt quá',
    playful: 'Không ổn lắm',
    calm: 'Mình cần nghỉ',
    shy: 'Khó chịu quá',
    grumpy: 'Đau quá',
    energetic: 'Không còn sức',
  },
  worried: {
    affectionate: 'Lo quá',
    playful: 'Có chuyện gì vậy',
    calm: 'Bình tĩnh nào',
    shy: 'Sợ một chút',
    grumpy: 'Căng thẳng quá',
    energetic: 'Nguy rồi',
  },
  angry: {
    affectionate: 'Mình giận đó',
    playful: 'Không vui đâu',
    calm: 'Dừng lại nhé',
    shy: 'Bực thật',
    grumpy: 'Hừ!',
    energetic: 'Tức quá đi',
  },
};

const levelSuffixVi: Record<'l1' | 'l2' | 'l3', string[]> = {
  l1: ['!', ' nha!', ' nè!'],
  l2: ['!!', ' đó!', ' nha!'],
  l3: ['!!!', ' luôn!', ' thật đấy!'],
};

const buildEmotionTextVi = () => {
  const result: any = {};
  for (const [mood, moodValue] of Object.entries(emotionEmojiVi)) {
    result[mood] = {};
    for (const [level, levelValue] of Object.entries(moodValue)) {
      result[mood][level] = {};
      for (const [tone, emojis] of Object.entries(levelValue as Record<string, string[]>)) {
        const stem = moodToneStemsVi[mood]?.[tone] || 'Mình ổn mà';
        const suffixes = levelSuffixVi[level as 'l1' | 'l2' | 'l3'] || levelSuffixVi.l1;
        result[mood][level][tone] = emojis.map(
          (emoji, idx) => `${emoji} ${stem}${suffixes[idx % suffixes.length]}`
        );
      }
    }
  }
  return result;
};

const emotionTextVi = buildEmotionTextVi();


export const viVN = {
  profile: {
    title: 'Jello của tôi',
    home: 'Trang chủ',
    signedInAs: 'Đăng nhập với',
    guestUser: 'Khách',
    status: {
      premium: '👑 CAO CẤP',
      angelPass: 'Thẻ Thiên Thần',
      jelloPass: 'Jello Pass',
      free: 'DÙNG THỬ',
      premiumLabel: '✨ Thành viên cao cấp',
      freeLabel: '🌱 Gói miễn phí',
    },
    cancelSubscription: 'Huỷ đăng ký',
    cancelConfirmTitle: 'Huỷ đăng ký gói?',
    cancelConfirmMessage: 'Premium sẽ vẫn hoạt động cho đến khi gói hiện tại kết thúc.\nSẽ không có khoản phí nào nữa.',
    cancelSuccess: 'Đã huỷ đăng ký.',
    cancellationScheduledBadge: 'Đã tắt tự động gia hạn',
    cancellationScheduledMessage: 'Premium sẽ vẫn hoạt động cho đến khi gói hiện tại kết thúc.',
    upgradePrompt: 'Nâng cấp',
    premiumTitle: 'Lon len cung Angel Pass',
    premiumSubtitle: 'Them nhieu phat trien cho con ban,\nthem nhieu hy vong cho mot em nho dang can giup do.',
    impactCard: {
      label: 'Tac dong toan cau',
      title: '2.450 thien than dang cung lon len',
    },
    subscription: {
      quarterly: {
        subscriptionTitle: 'Goi Jello theo quy',
        subscriptionDesc: '$3.99, thanh toan theo quy',
        durationTitle: 'Goi Jello 3 thang',
        durationDesc: '$3.99',
      },
      yearly: {
        subscriptionTitle: 'Goi Angel hang nam',
        subscriptionDesc: '$12.00, thanh toan hang nam',
        durationTitle: 'Goi Angel 12 thang',
        durationDesc: '$12.00',
        badge: 'Tiết kiệm nhất',
      },
      unit: '/ tháng',
      currency: 'USD',
    },
    cancelPolicy: 'Có thể huỷ mọi gói bất cứ lúc nào.',
    premiumActive: {
      title: 'Bạn đang là thành viên Premium!',
      desc: 'Cảm ơn bạn đã ủng hộ.',
    },
    myJelloBox: 'Bộ sưu tập Jello',
    collectionLabel: 'Bộ sưu tập',
    profileInfo: 'Tài khoản',
    paymentInfo: 'Thanh toán',
    checkoutKicker: 'Thanh toán an toàn',
    checkoutAriaLabel: 'Hộp thoại thanh toán an toàn',
    checkoutTitle: 'Mua gói',
    checkoutPreparing: 'Đang chuẩn bị thanh toán...',
    purchaseButton: 'Mua',
    expiresLabel: 'Het han',
    purchaseConfirm: {
      threeMonths: 'Xac nhan mua goi 3 thang?',
      twelveMonths: 'Xac nhan mua goi 1 nam?',
    },
    purchaseResult: {
      success: 'Mua thanh cong.',
      failure: 'Mua that bai.',
      failureDetailed: 'Chua the hoan tat giao dich. Vui long thu lai sau it phut.',
    },
    cancelFailure: 'Chua the huy goi luc nay. Vui long thu lai sau it phut.',
    cancelErrors: {
      alreadyInProgress: 'Yeu cau huy dang duoc xu ly. Vui long doi mot chut.',
      rateLimited: 'Co qua nhieu yeu cau. Vui long doi truoc khi thu lai.',
      managedExternally: 'Hay huy goi nay tren Xsolla va cho cap nhat da duoc xac minh.',
    },
    cancelAnytimeShort: 'Co the huy bat cu luc nao',
    oneTimePurchaseNote: 'Khong tu dong gia han',
    badges: {
      recommended: 'Đề xuất',
      subscription: 'Gói thuê bao',
      mostPopular: 'Pho bien',
    },
    tabs: {
      myJello: 'Jello Của Tôi',
      pass: 'Vé Thiên Thần',
    },
    parentGate: {
      title: 'Cho Angel Pass',
      instruction: 'Nhập các số hiển thị bên dưới để truy cập',
      error: 'Chưa đúng. Vui lòng thử lại.',
      clear: 'Xóa',
      reset: 'Mã mới',
      displayLabel: 'Các số đã nhập',
      numberWords: {
        1: 'một',
        2: 'hai',
        3: 'ba',
        4: 'bốn',
        5: 'năm',
        6: 'sáu',
        7: 'bảy',
        8: 'tám',
        9: 'chín',
      },
    },
  },
  landing: {
    title: 'Nuôi dưỡng Jello',
    subtitle: 'Lớn lên cùng người bạn đáng yêu',
    secret_invitation: 'Lời mời bí mật đến Thế giới Jello',
    portal_chip_raise: '🥚 nuôi dưỡng',
    portal_chip_play: '🎮 vui chơi',
    portal_chip_learn: '📚 học hỏi',
    free_start: 'Bắt đầu miễn phí',
    start_experience: 'Bắt đầu ngay',
    auth_prompt: 'Đã có tài khoản?',
    login: 'Đăng nhập',
    signup: 'Đăng ký',
    back_to_home: 'Về trang chủ',
    continue_experience: 'Tiếp tục',
    new_game: 'Mới',
    auth: {
      duplicateLoginAlert: 'Tài khoản đã đăng nhập ở thiết bị khác. Phiên hiện tại sẽ bị đóng.',
    },
  },

  character: {
    profile: {
      level: 'Lv.{{level}}',
    },
    stats: {
      hunger: 'Đói bụng',
      happiness: 'Hạnh phúc',
      health: 'Sức khỏe',
      hygiene: 'Vệ sinh',
      fatigue: 'Mệt mỏi',
      affection: 'Tình cảm',
    },
    tags: {
      defense: 'Phòng thủ',
      honey_wood: 'Mật ong/Gỗ',
      attack: 'Tấn công',
      fire_flame: 'Lửa/Ngọn lửa',
      magic: 'Phép thuật',
      poison_curse: 'Độc/Lời nguyền',
      sleep: 'Giấc ngủ',
      illusion_cottoncandy: 'Ảo ảnh/Kẹo bông',
      hp: 'HP',
      vitality_buff: 'Sinh lực/Buff',
      heal: 'Hồi phục',
      purification_plant: 'Thanh lọc/Cây cỏ',
      bind: 'Trói buộc',
      stability_normal: 'Ổn định/Bình thường',
      speed: 'Tốc độ',
      flexibility_water: 'Linh hoạt/Nước',
    },
    species: {
      yellowJello: 'Jello Vàng',
      redJello: 'Jello Đỏ',
      limeJello: 'Jello Chanh',
      mintJello: 'Jello Bạc hà',
      blueJello: 'Jello Xanh dương',
      creamJello: 'Jello Kem',
      pinkJello: 'Jello Hồng',
      purpleJello: 'Jello Tím',
      skyJello: 'Jello Xanh trời',
      brownJello: 'Jello Nâu',
      orangeJello: 'Jello Cam',
      oliveJello: 'Jello Ô liu',
      cyanJello: 'Jello Lục lam',
    },
    evolutions: {
      // Yellow Jello
      yellowJello_stage1_name: 'Jello Vàng',
      yellowJello_stage1_desc: 'Một bé thạch có mùi ngọt dịu. Trông như giọt nước nhỏ giữa khu rừng.',
      yellowJello_stage2_name: 'Twiglo',
      yellowJello_stage2_desc: 'Một cành non nhỏ mọc trên đầu. Nhựa ngọt từ cành cây thu hút côn trùng.',
      yellowJello_stage3_name: 'Vinegel',
      yellowJello_stage3_desc: 'Được quấn bởi dây leo cứng cáp nên phòng thủ rất cao. Nó dùng mật dính giữa các dây leo để giữ chân kẻ địch.',
      yellowJello_stage4_name: 'Honeybloom',
      yellowJello_stage4_desc: 'Cơ thể nở hoa và mọc thêm tay chân. Nó đi cùng người bạn ong mật và chia sẻ mật để hồi phục thể lực.',
      yellowJello_stage5_name: 'Arboros',
      yellowJello_stage5_desc: 'Rồng Hộ Vệ của Đại Ngàn. Nó bay bằng cánh lá và dùng "Hơi Thở Mật Ong" từ bông hoa trên đuôi để ru ngủ kẻ địch.',

      // Red Jello
      redJello_stage1_name: 'Jello Đỏ',
      redJello_stage1_desc: 'Một bé thạch chạm vào thấy ấm nóng. Khi tức giận, màu đỏ đậm hơn và xung quanh nóng lên.',
      redJello_stage2_name: 'Devilet',
      redJello_stage2_desc: 'Cặp sừng đỏ đáng yêu mọc trên đầu. Nó thích chọc mọi thứ bằng sừng, và sẽ tóe tia lửa khi nổi nóng.',
      redJello_stage3_name: 'Impgel',
      redJello_stage3_desc: 'Nó bay nhanh nhẹn bằng đôi cánh dơi. Thích bày trò tinh nghịch rồi bỏ chạy, nhưng sẽ ngoan lại nếu được cho kẹo.',
      redJello_stage4_name: 'Pyron',
      redJello_stage4_desc: 'Ngọn lửa không tắt cháy trên đuôi. Nó có thể phun lửa và luôn xông lên trước để bảo vệ bạn bè yếu hơn.',
      redJello_stage5_name: 'Infernos',
      redJello_stage5_desc: 'Chúa Tể Địa Ngục với dung nham cuộn chảy. Là hiện thân của một "Lời Thề", tự trói mình bằng xiềng xích để kìm sức hủy diệt.',

      // Mint Jello
      mintJello_stage1_name: 'Jello Bạc hà',
      mintJello_stage1_desc: 'Một bé thạch mát lạnh, tươi mới khi chạm vào. Nó hấp thụ sương trong và để lại hương thảo mộc dễ chịu.',
      mintJello_stage2_name: 'Leaflo',
      mintJello_stage2_desc: 'Một mầm non đáng yêu xuất hiện trên đầu. Nó yêu nước trong và ánh nắng, luôn tìm chỗ có nắng.',
      mintJello_stage3_name: 'Bloomint',
      mintJello_stage3_desc: 'Mầm cây đã lớn thành một bông hoa xinh đẹp. Nó vỗ lá để thanh lọc không khí và giúp bạn bè nghỉ ngơi.',
      mintJello_stage4_name: 'Fairyleaf',
      mintJello_stage4_desc: 'Nó bay khắp rừng bằng đôi cánh lá lớn. Chỉ cần đến gần, nó có thể làm cây cối héo úa hồi sinh.',
      mintJello_stage5_name: 'Verdantos',
      mintJello_stage5_desc: 'Rồng Hộ Vệ Khu Rừng, khoác lên sức sống của thiên nhiên. Đôi cánh của nó tạo ra "Gió Hồi Phục" chữa lành trên diện rộng.',

      // Blue Jello
      blueJello_stage1_name: 'Jello Xanh dương',
      blueJello_stage1_desc: 'Một bé thạch trong suốt và mát như nước biển sâu. Ít biểu cảm nhưng thích nghi linh hoạt như nước.',
      blueJello_stage2_name: 'Fingel',
      blueJello_stage2_desc: 'Một vây cá mập mượt mà xuất hiện trên đầu. Nó bơi cực nhanh dưới nước và thoát hiểm tức thì khi nguy cấp.',
      blueJello_stage3_name: 'Coralin',
      blueJello_stage3_desc: 'Những cụm san hô rực màu xuất hiện trên cơ thể. Nó có thể thở dưới nước và cơ thể đủ cứng để chịu áp lực nước.',
      blueJello_stage4_name: 'Hydro',
      blueJello_stage4_desc: 'Nó có cánh dạng vây và bắn "Pháo Nước" từ miệng. Nó thích thiền định ở vùng nước sâu.',
      blueJello_stage5_name: 'Oceanos',
      blueJello_stage5_desc: 'Rồng Nước cai quản đại dương. Nó có lớp vảy trong suốt phát sáng và có thể gây sóng thần khổng lồ khi nổi giận.',

      // Cream Jello
      creamJello_stage1_name: 'Jello Kem',
      creamJello_stage1_desc: 'Một bé thạch ấm áp với mùi bánh mì mới nướng. Tính cách thong thả, hay thấy đang lim dim ở chỗ có nắng.',
      creamJello_stage2_name: 'Mewlo',
      creamJello_stage2_desc: 'Tai mèo mọc lên trên đầu. Nó nghe rất nhạy và bật dậy ngay khi nghe tiếng mở gói đồ ăn vặt.',
      creamJello_stage3_name: 'Whiskel',
      creamJello_stage3_desc: 'Với ria và chiếc đuôi dài, nó có thể ngồi "ổ bánh mì" rất chuẩn. Nó thích hộp chật và sẽ quật đuôi xuống sàn khi bực.',
      creamJello_stage4_name: 'Keepurr',
      creamJello_stage4_desc: 'Lười đến mức hầu như không muốn di chuyển, luôn giữ tư thế "ổ bánh mì". Cá trên đầu và cuộn len ở đuôi là kho báu nó bảo vệ bằng tiếng khè.',
      creamJello_stage5_name: 'Felinos',
      creamJello_stage5_desc: 'Một Rồng Mèo thanh lịch. Dấu chân thạch trên cánh khiến đối thủ mất cảnh giác trước khi nhận "Cú Đấm Meow" uy lực.',

      // Pink Jello
      pinkJello_stage1_name: 'Jello Hồng',
      pinkJello_stage1_desc: 'Một bé thạch hồng thơm mùi dâu tây. Khi vui, nó phồng lên và tỏa hương ngọt ngào.',
      pinkJello_stage2_name: 'Lolligel',
      pinkJello_stage2_desc: 'Cặp râu kẹo mút xuất hiện trên đầu. Nó lắc râu để an ủi những người bạn đang buồn.',
      pinkJello_stage3_name: 'Twingel',
      pinkJello_stage3_desc: 'Những viên kẹo sao lấp lánh trang trí trên cơ thể. Mỗi lần di chuyển đều phát ra tiếng "pop" trong trẻo.',
      pinkJello_stage4_name: 'Cottonpuff',
      pinkJello_stage4_desc: 'Nó lơ lửng cùng những đám mây kẹo bông mềm xốp. Bên trong kẹo bông là đủ món ăn vặt ngon lành.',
      pinkJello_stage5_name: 'Candios',
      pinkJello_stage5_desc: 'Rồng Kỳ Ảo với đôi cánh kẹo bông. Lớp bụi ngọt nó rải ra khiến kẻ địch chìm vào giấc ngủ sâu đầy hạnh phúc.',



      // Authrple Jello
      purpleJello_stage1_name: 'Jello Tím',
      purpleJello_stage1_desc: 'Một bé thạch phát sáng trong đêm. Nó lẩm nhẩm như đang niệm những câu thần chú bí ẩn.',
      purpleJello_stage2_name: 'Wisper',
      purpleJello_stage2_desc: 'Một đốm lửa ma xuất hiện trên đuôi. Nó dùng ánh sáng này để giao tiếp với linh hồn và soi lối trong bóng tối.',
      purpleJello_stage3_name: 'Runeimp',
      purpleJello_stage3_desc: 'Một chiếc sừng khắc cổ ngữ mọc trên trán. Nó dùng ánh sáng đuôi để dụ và làm đối thủ rối loạn.',
      purpleJello_stage4_name: 'Spellbound',
      purpleJello_stage4_desc: 'Cơ thể quấn đầy bùa chú để kiểm soát ma lực. Người bạn thân nhất của nó là một chiếc đầu lâu luôn canh chừng phía sau.',
      purpleJello_stage5_name: 'Arcanios',
      purpleJello_stage5_desc: 'Rồng Pháp Thuật cổ xưa đã phá phong ấn. Kẻ dẫn đường cõi âm có thể ban lời nguyền hoặc phúc lành chỉ bằng một nhịp cánh.',

      // Orange Jello
      orangeJello_stage1_name: 'Jello Cam',
      orangeJello_stage1_desc: 'Một bé thạch thơm mát như nước ép mới vắt. Nhờ độ đàn hồi cao, nó rất thích nảy tưng tưng như quả bóng.',
      orangeJello_stage2_name: 'Mandapop',
      orangeJello_stage2_desc: 'Đôi tai như múi quýt xuất hiện. Khi vui, bong bóng lăn tăn như soda nổi lên. Nó rất giỏi lăn tròn.',
      orangeJello_stage3_name: 'Citrupix',
      orangeJello_stage3_desc: 'Một mầm cây và đôi cánh nhỏ mọc lên trên đầu. Nó quang hợp dưới nắng và lan tỏa nguồn năng lượng sảng khoái xung quanh.',
      orangeJello_stage4_name: 'Vitawing',
      orangeJello_stage4_desc: 'Một "Quả Quýt Sinh Mệnh" giàu dưỡng chất mọc trên đuôi. Nó chia sẻ nước ép cho đồng đội kiệt sức để hồi sinh lực.',
      orangeJello_stage5_name: 'Tangeros',
      orangeJello_stage5_desc: 'Rồng Trái Cây với sức sống bền bỉ. "Hơi Thở Cam Quýt" của nó tươi mát đến mức khiến đối thủ mất ý chí chiến đấu.',

      // Sky Jello
      skyJello_stage1_name: 'Jello Xanh trời',
      skyJello_stage1_desc: 'Jello màu trời xanh trong trẻo.',

      // Brown Jello
      brownJello_stage1_name: 'Jello Nâu',
      brownJello_stage1_desc: 'Jello màu nâu ấm áp.',

      // Olive Jello
      oliveJello_stage1_name: 'Jello Ô liu',
      oliveJello_stage1_desc: 'Jello màu ô liu trầm nhẹ.',

      // Cyan Jello
      cyanJello_stage1_name: 'Jello Lục lam',
      cyanJello_stage1_desc: 'Jello màu lục lam mát dịu.',
    },
  },
  common: {
    startGame: 'Bắt đầu trò chơi',
    start: 'Bắt đầu',
    cancel: 'Huỷ',
    premium: 'Cao cấp',
    upgrade_btn_text: 'NÂNG CẤP',
    modal: {
      title: 'Một cho bạn,\nmột cho người bạn cần giúp đỡ.',
      desc: '<bold>Một Thẻ Thiên Thần</bold>,<br/>hai hành trình học tập.',
      benefit1: 'Mở khóa toàn bộ tiến hóa Jello và trò chơi',
      benefit2: 'Tang goi 1 nam cho mot em nho can giup do',
      benefit3: 'Phat trien nhanh hon voi tang cuong XP & GRO',
    },
    loading: 'Đang tải...',
    confirm: 'Xác nhận',
    close: 'Đóng',
    menu: 'Trình đơn',
    score: 'Điểm',
    lives: 'Mạng',
    combo: 'Chuỗi',
    bestCombo: 'CHUỖI CAO NHẤT',
    difficulty: 'Độ khó',
    time: 'Thời gian',
    accuracy: 'Độ chính xác',
    stageClear: 'Hoàn thành màn!',
    gameOver: 'Trò chơi kết thúc!',
    finalScore: 'Điểm cuối',
    bestScore: 'Điểm cao nhất',
    previousBest: 'Kỷ lục trước',
    newRecord: 'KỶ LỤC MỚI!',
    playAgain: 'Chơi lại',
    download: 'Kết quả',
    howToPlay: 'Cách chơi',
    results: 'Kết quả',
    earnedXp: 'XP',
    earnedGro: 'Gro',
    earnedStar: 'Sao',
    yes: 'Có',
    no: 'Không',
    confirm_reset: 'Bạn có chắc muốn bắt đầu trò chơi mới không? Dữ liệu hiện tại sẽ bị mất.',
    goHome: 'Về trang chủ',
    errorBoundary: {
      updateTitle: 'Phiên bản mới đã được phát hành!',
      updateDesc: 'Vui lòng tải lại trang này để dùng phiên bản mới nhất.',
      refreshButton: 'Tải lại',
      tempErrorTitle: 'Đã xảy ra lỗi tạm thời.',
      tempErrorRetry: 'Thử lại',
      globalErrorTitle: 'Đã xảy ra lỗi.',
      globalErrorDesc: 'Xin lỗi, đã có sự cố khi chạy ứng dụng.',
      errorDetailsTitle: 'Chi tiết lỗi',
    },
  },
  evolution: {
    title: 'TIẾN HÓA!',
    continue: 'Chạm để tiếp tục',
  },
  graduation: {
    title: 'Tạm biệt, Jello!',
    message: 'Jello của bạn đã trưởng thành và lên đường khám phá thế giới! 🌍',
    action: 'Hẹn gặp lại!',
  },
  food: {
    menu: {
      title: 'Thức ăn',
      close: 'Đóng',
    },
    categories: {
      fruit: 'Trái cây',
      vegetable: 'Rau củ',
      bakery: 'Bánh mì',
      meal: 'Món chính',
      snack: 'Ăn vặt',
      dessert: 'Tráng miệng',
    },
    items: {
      // Fruits
      grapes: 'Nho',
      melon: 'Dưa lưới',
      watermelon: 'Dưa hấu',
      tangerine: 'Quýt',
      lemon: 'Chanh vàng',
      lime: 'Chanh',
      banana: 'Chuối',
      pineapple: 'Dứa',
      mango: 'Xoài',
      red_apple: 'Táo đỏ',
      green_apple: 'Táo xanh',
      pear: 'Lê',
      peach: 'Đào',
      cherries: 'Cherry',
      strawberry: 'Dâu tây',
      blueberries: 'Việt quất',
      kiwi_fruit: 'Kiwi',
      tomato: 'Cà chua',
      olive: 'Ô liu',
      coconut: 'Dừa',

      // Vegetables
      avocado: 'Bơ',
      eggplant: 'Cà tím',
      potato: 'Khoai tây',
      carrot: 'Cà rốt',
      ear_of_corn: 'Bắp',
      hot_pepper: 'Ớt cay',
      bell_pepper: 'Ớt chuông',
      cucumber: 'Dưa leo',
      leafy_green: 'Rau lá xanh',
      broccoli: 'Bông cải xanh',
      garlic: 'Tỏi',
      onion: 'Hành tây',
      peanuts: 'Đậu phộng',
      beans: 'Đậu',
      chestnut: 'Hạt dẻ',
      ginger: 'Gừng',
      pea_pod: 'Đậu Hà Lan',
      mushroom: 'Nấm',

      // Bakery
      bread: 'Bánh mì',
      croissant: 'Bánh sừng bò',
      baguette_bread: 'Baguette',
      flatbread: 'Bánh mì dẹt',
      pretzel: 'Bánh xoắn',
      bagel: 'Bagel',
      pancakes: 'Bánh pancake',
      waffle: 'Waffle',
      butter: 'Bơ',
      cheese_wedge: 'Phô mai miếng',

      // Meals
      meat_on_bone: 'Thịt xương',
      poultry_leg: 'Đùi gà',
      cut_of_meat: 'Miếng thịt',
      bacon: 'Thịt xông khói',
      hamburger: 'Hamburger',
      french_fries: 'Khoai tây chiên',
      pizza: 'Pizza',
      hot_dog: 'Hot dog',
      sandwich: 'Sandwich',
      taco: 'Taco',
      burrito: 'Burrito',
      stuffed_flatbread: 'Bánh kẹp nhân',
      falafel: 'Falafel',
      egg: 'Trứng',
      cooking: 'Món nấu',
      curry_rice: 'Cơm cà ri',
      steaming_bowl: 'Tô nóng hổi',
      cooked_rice: 'Cơm',
      pot_of_food: 'Nồi món ăn',
      shallow_pan_of_food: 'Chảo món ăn',

      // Snacks
      oden: 'Oden',
      rice_cracker: 'Bánh gạo',
      rice_ball: 'Cơm nắm',
      fried_shrimp: 'Tôm chiên',
      fish_cake_with_swirl: 'Chả cá xoắn',
      dumpling: 'Há cảo',
      fortune_cookie: 'Bánh may mắn',
      moon_cake: 'Bánh trung thu',
      takeout_box: 'Hộp mang về',
      popcorn: 'Bắp rang bơ',
      canned_food: 'Đồ hộp',
      roasted_sweet_potato: 'Khoai lang nướng',
      tamale: 'Tamale',

      // Desserts
      soft_ice_cream: 'Kem tươi',
      shaved_ice: 'Đá bào',
      ice_cream: 'Kem',
      doughnut: 'Bánh donut',
      cookie: 'Bánh quy',
      birthday_cake: 'Bánh sinh nhật',
      shortcake: 'Bánh shortcake',
      cupcake: 'Cupcake',
      pie: 'Bánh pie',
      chocolate_bar: 'Socola thanh',
      candy: 'Kẹo',
      lollipop: 'Kẹo mút',
      custard: 'Bánh flan',
      honey_pot: 'Mật ong',
    },
    effects: {
      hunger: 'No',
      happiness: 'Hạnh phúc',
      health: 'Sức khỏe',
    },
  },
  medicine: {
    menu: {
      title: 'Thuốc',
    },
    items: {
      pill: 'Viên thuốc',
      syringe: 'Tiêm',
    },
  },
  nurturingPanel: {
    stats: {
      fullness: 'Do no',
      health: 'Suc khoe',
      happiness: 'Hanh phuc',
    },
    alerts: {
      hungry: '🍖 Tôi đói quá! Cho tôi ăn nhé',
      sick: '💊 Tôi thấy mệt! Tôi cần thuốc',
    },
    interactions: {
      catchBug: 'Chạm để bắt',
      cleanPoop: 'Chạm để dọn',
    },
    studyCount: 'Học {{count}} lần',
  },
  shop: {
    menu: {
      title: 'Cửa hàng',
    },
    status: {
      active: 'Đang dùng',
      owned: 'Đã sở hữu',
      free: 'Miễn phí',
    },
    categories: {
      ground: 'Nền',
      house: 'Nhà',
      pet: 'Thú cưng',
    },
    items: {
      default_ground: 'Phòng ấm cúng',
      'default_ground.desc': 'Phòng bắt đầu ấm áp và dễ chịu.',
      forest_ground: 'Nền rừng',
      'forest_ground.desc': 'Không khí trong lành và cây xanh.',
      tropical_ground: 'Bãi biển nhiệt đới',
      'tropical_ground.desc': 'Thư giãn dưới hàng dừa.',
      desert_ground: 'Nền sa mạc',
      'desert_ground.desc': 'Nắng nóng và đồi cát.',
      arctic_ground: 'Nền băng giá',
      'arctic_ground.desc': 'Băng giá lạnh và cực quang.',
      volcanic_ground: 'Nền núi lửa',
      'volcanic_ground.desc': 'Dung nham và tro núi lửa.',
      shape_ground: 'Nền hình học',
      'shape_ground.desc': 'Thế giới đầy kỳ quan hình học.',
      sweet_ground: 'Nền kẹo ngọt',
      'sweet_ground.desc': 'Thế giới ngọt ngào như mơ.',
      night_city: 'Thành phố đêm',
      'night_city.desc': 'Thành phố không bao giờ ngủ.',
      deep_sea_ground: 'Đáy biển',
      'deep_sea_ground.desc': 'Thế giới dưới nước đầy bí ẩn.',
      amusement_park_ground: 'Công viên giải trí mơ mộng',
      'amusement_park_ground.desc': 'Xứ sở vui chơi đầy trò chơi thú vị.',
      layout1_template: 'Mẫu bố cục 1',
      'layout1_template.desc': 'Mẫu để tạo nền mới (Tường 50%, Sàn 80%).',

      // Houses
      tent: 'Lều',
      'tent.desc': 'Lều cắm trại ấm cúng.',
      old_house: 'Nhà cũ',
      house: 'Nhà',
      garden_house: 'Nhà vườn',
      building: 'Tòa nhà',
      hotel: 'Khách sạn',
      factory: 'Nhà máy',
      circus: 'Rạp xiếc',
      stadium: 'Sân vận động',
      church: 'Nhà thờ',
      mosque: 'Đền Hồi giáo',
      hindu_temple: 'Đền Hindu',
      synagogue: 'Giáo đường Do Thái',
      greek_temple: 'Đền Hy Lạp',
      kaaba: 'Kaaba',
      japanese_castle: 'Lâu đài Nhật Bản',
      european_castle: 'Lâu đài Châu Âu',

      // Pet
      'pet.partner': 'Bạn đồng hành',
      'pet.timeRemaining': 'Thời gian còn lại',
      'pet.expired': 'Đã hết hạn',
      'pet.gacha.title': 'Thú cưng ngẫu nhiên',
      'pet.gacha.desc': 'Thú cưng sẽ đến chơi với bạn một lúc',
      'pet.gacha.price': '💰 350',

      // Pet Names
      'pet.monkey': 'Ooky',
      'pet.hedgehog': 'Prickles',
      'pet.lizard': 'Zippy',
      'pet.octopus': 'Inky',
      'pet.squid': 'Wiggly',
      'pet.snail': 'Shelly',
      'pet.scorpion': 'Stinger',
      'pet.turtle': 'Tank',
      'pet.dodo': 'Waddle',
      'pet.snowman': 'Frosty',
      'pet.dino': 'Roar',
      'pet.phoenix': 'Blaze',
      'pet.r2_pet_1': 'Special Pet',
    },
  },
  rarity: {
    common: 'Phổ biến',
    uncommon: 'Không phổ biến',
    rare: 'Hiếm',
  },
  actions: {
    feed: 'Cho ăn',
    medicine: 'Thuốc',
    play: 'Chơi',
    clean: 'Dọn dẹp',
    camera: 'Máy ảnh',
    settings: 'Cài đặt',
    evolve: 'Tiến hóa',
    graduate: 'Tốt nghiệp',
    cancel: 'Huỷ',
    confirm: 'Xác nhận',
    close: 'Đóng',
  },
  camera: {
    title: 'Chụp ảnh!',
    save: 'Lưu',
    copyLink: 'Sao chép liên kết',
    capturing: 'Đang chụp...',
  },
  share: {
    share: 'Chia sẻ',
    linkCopied: 'Đã sao chép liên kết!',
    copyFailed: 'Sao chép thất bại',
    invite: {
      title: 'Đây là Jello mình đang nuôi! 🥰',
      desc: 'Bạn muốn nuôi cùng không?',
    },
    cta: 'Chơi ngay!',
    error: {
      invalid: 'Liên kết chia sẻ không hợp lệ',
      missing: 'Không tìm thấy dữ liệu',
    },
  },
  sleep: {
    confirm: {
      sleepTitle: 'Đi ngủ',
      sleepMessage: 'Bạn muốn cho Jello đi ngủ chứ? 💤\n(Ngủ trong 30 phút)',
      wakeTitle: 'Thức dậy',
      wakeMessage: 'Bạn muốn đánh thức Jello chứ? 🌅',
    },
  },
  cleanMenu: {
    title: 'Dụng cụ dọn dẹp',
    broom: {
      name: 'Chổi',
      effect: 'Dọn 1 bục bẩn',
    },
    newspaper: {
      name: 'Báo cũ',
      effect: 'Bắt 1 con sâu',
    },
    shower: {
      name: 'Tắm rửa',
      effect: 'Sạch sẽ hơn',
    },
    robot_cleaner: {
      name: 'Robot dọn dẹp',
      effect: 'Dọn sạch tất cả',
    },
    toothbrush: {
      name: 'Bàn chải',
      effect: 'Sạch sẽ hơn',
    },
    max_stats: {
      name: 'tất cả',
      effect: '+100',
    },
  },
  emotions: {
    ...emotionTextVi,
    emoji: emotionEmojiVi,
        toddler: {
          "joy": {
                "l1": {
                      "affectionate": [
                            "😊 vui quá~",
                            "🍼 vui quá nha~",
                            "✨ vui quá nè~"
                      ],
                      "playful": [
                            "😆 chơi vui~",
                            "🎶 chơi vui nha~",
                            "😁 chơi vui nè~"
                      ],
                      "calm": [
                            "😌 êm ghê~",
                            "🎵 êm ghê nha~",
                            "👌 êm ghê nè~"
                      ],
                      "shy": [
                            "☺️ ngại quá~",
                            "😳 ngại quá nha~",
                            "😚 ngại quá nè~"
                      ],
                      "grumpy": [
                            "😒 hừm~",
                            "😏 hừm nha~",
                            "🙄 hừm nè~"
                      ],
                      "energetic": [
                            "✨ đi thôi~",
                            "👍 đi thôi nha~",
                            "😲 đi thôi nè~"
                      ]
                },
                "l2": {
                      "affectionate": [
                            "🥰 vui quá!",
                            "🌈 vui quá nha!",
                            "🎈 vui quá nè!"
                      ],
                      "playful": [
                            "🏃 chơi vui!",
                            "⚽️ chơi vui nha!",
                            "🦖 chơi vui nè!"
                      ],
                      "calm": [
                            "☕️ êm ghê!",
                            "🍀 êm ghê nha!",
                            "🍃 êm ghê nè!"
                      ],
                      "shy": [
                            "😽 ngại quá!",
                            "🌸 ngại quá nha!",
                            "🙈 ngại quá nè!"
                      ],
                      "grumpy": [
                            "😑 hừm!",
                            "😼 hừm nha!",
                            "😮 hừm nè!"
                      ],
                      "energetic": [
                            "🏆 đi thôi!",
                            "💪 đi thôi nha!",
                            "🚀 đi thôi nè!"
                      ]
                },
                "l3": {
                      "affectionate": [
                            "❤️ vui quá!!",
                            "💖 vui quá luôn!",
                            "🌟 vui quá quá!"
                      ],
                      "playful": [
                            "💡 chơi vui!!",
                            "👑 chơi vui luôn!",
                            "😂 chơi vui quá!"
                      ],
                      "calm": [
                            "🙏 êm ghê!!",
                            "🙇 êm ghê luôn!",
                            "👏 êm ghê quá!"
                      ],
                      "shy": [
                            "🍅 ngại quá!!",
                            "🎁 ngại quá luôn!",
                            "🦄 ngại quá quá!"
                      ],
                      "grumpy": [
                            "🆗 hừm!!",
                            "💬 hừm luôn!",
                            "😤 hừm quá!"
                      ],
                      "energetic": [
                            "🔥 đi thôi!!",
                            "🌍 đi thôi luôn!",
                            "📢 đi thôi quá!"
                      ]
                }
          },
          "love": {
                "l1": {
                      "affectionate": [
                            "😘 thương lắm~",
                            "🐣 thương lắm nha~",
                            "👋 thương lắm nè~"
                      ],
                      "playful": [
                            "🎣 ôm~",
                            "💎 ôm nha~",
                            "👉 ôm nè~"
                      ],
                      "calm": [
                            "🌤️ an tâm rồi~",
                            "👤 an tâm rồi nha~",
                            "🛡️ an tâm rồi nè~"
                      ],
                      "shy": [
                            "😺 tim đập nhanh~",
                            "🧣 tim đập nhanh nha~",
                            "😊 tim đập nhanh nè~"
                      ],
                      "grumpy": [
                            "😕 dỗi rồi~",
                            "✋ dỗi rồi nha~",
                            "😤 dỗi rồi nè~"
                      ],
                      "energetic": [
                            "💘 yêu quá~",
                            "🤜🤛 yêu quá nha~",
                            "👍 yêu quá nè~"
                      ]
                },
                "l2": {
                      "affectionate": [
                            "🤗 thương lắm!",
                            "🌞 thương lắm nha!",
                            "💋 thương lắm nè!"
                      ],
                      "playful": [
                            "👐 ôm!",
                            "🐨 ôm nha!",
                            "🏹 ôm nè!"
                      ],
                      "calm": [
                            "💎 an tâm rồi!",
                            "🌲 an tâm rồi nha!",
                            "🤝 an tâm rồi nè!"
                      ],
                      "shy": [
                            "☺️ tim đập nhanh!",
                            "💌 tim đập nhanh nha!",
                            "💓 tim đập nhanh nè!"
                      ],
                      "grumpy": [
                            "😳 dỗi rồi!",
                            "⏱️ dỗi rồi nha!",
                            "😒 dỗi rồi nè!"
                      ],
                      "energetic": [
                            "⚡️ yêu quá!",
                            "🤖 yêu quá nha!",
                            "♾️ yêu quá nè!"
                      ]
                },
                "l3": {
                      "affectionate": [
                            "🏡 thương lắm!!",
                            "👫 thương lắm luôn!",
                            "💝 thương lắm quá!"
                      ],
                      "playful": [
                            "👮 ôm!!",
                            "🎖️ ôm luôn!",
                            "🫡 ôm quá!"
                      ],
                      "calm": [
                            "💐 an tâm rồi!!",
                            "🧶 an tâm rồi luôn!",
                            "✨ an tâm rồi quá!"
                      ],
                      "shy": [
                            "🫂 tim đập nhanh!!",
                            "😻 tim đập nhanh luôn!",
                            "🍀 tim đập nhanh quá!"
                      ],
                      "grumpy": [
                            "😎 dỗi rồi!!",
                            "😠 dỗi rồi luôn!",
                            "🐕 dỗi rồi quá!"
                      ],
                      "energetic": [
                            "👯 yêu quá!!",
                            "🚀 yêu quá luôn!",
                            "🎉 yêu quá quá!"
                      ]
                }
          },
          "playful": {
                "l1": {
                      "affectionate": [
                            "😋 thích quá~",
                            "🍓 thích quá nha~",
                            "🍭 thích quá nè~"
                      ],
                      "playful": [
                            "🐊 vui ghê~",
                            "🍔 vui ghê nha~",
                            "😛 vui ghê nè~"
                      ],
                      "calm": [
                            "🥄 chậm thôi~",
                            "🍵 chậm thôi nha~",
                            "🥗 chậm thôi nè~"
                      ],
                      "shy": [
                            "🐹 hehe~",
                            "🍪 hehe nha~",
                            "🐿️ hehe nè~"
                      ],
                      "grumpy": [
                            "😐 đừng chọc~",
                            "🦴 đừng chọc nha~",
                            "😑 đừng chọc nè~"
                      ],
                      "energetic": [
                            "🍡 nhanh lên~",
                            "🍚 nhanh lên nha~",
                            "🥕 nhanh lên nè~"
                      ]
                },
                "l2": {
                      "affectionate": [
                            "🎡 thích quá!",
                            "🤣 thích quá nha!",
                            "😆 thích quá nè!"
                      ],
                      "playful": [
                            "🔫 vui ghê!",
                            "⚔️ vui ghê nha!",
                            "🛡️ vui ghê nè!"
                      ],
                      "calm": [
                            "🧐 chậm thôi!",
                            "🎲 chậm thôi nha!",
                            "😏 chậm thôi nè!"
                      ],
                      "shy": [
                            "🫣 hehe!",
                            "👉👈 hehe nha!",
                            "👻 hehe nè!"
                      ],
                      "grumpy": [
                            "🛑 đừng chọc!",
                            "😩 đừng chọc nha!",
                            "👋 đừng chọc nè!"
                      ],
                      "energetic": [
                            "🐇 nhanh lên!",
                            "🌀 nhanh lên nha!",
                            "💨 nhanh lên nè!"
                      ]
                },
                "l3": {
                      "affectionate": [
                            "🎪 thích quá!!",
                            "🎈 thích quá luôn!",
                            "😵 thích quá quá!"
                      ],
                      "playful": [
                            "🎆 vui ghê!!",
                            "🎺 vui ghê luôn!",
                            "🦸 vui ghê quá!"
                      ],
                      "calm": [
                            "🥺 chậm thôi!!",
                            "😲 chậm thôi luôn!",
                            "👏 chậm thôi quá!"
                      ],
                      "shy": [
                            "🤭 hehe!!",
                            "✨ hehe luôn!",
                            "🤩 hehe quá!"
                      ],
                      "grumpy": [
                            "🙀 đi chỗ khác.",
                            "😼 im lặng nha.",
                            "👀 đừng chọc quá!"
                      ],
                      "energetic": [
                            "📈 nhanh lên!!",
                            "🚄 nhanh lên luôn!",
                            "🏎️ nhanh lên quá!"
                      ]
                }
          },
          "neutral": {
                "l1": {
                      "affectionate": [
                            "🧸 ở với mình~",
                            "🥺 ở với mình nha~",
                            "💧 ở với mình nè~"
                      ],
                      "playful": [
                            "🤔 làm gì ta~",
                            "😈 làm gì ta nha~",
                            "💬 làm gì ta nè~"
                      ],
                      "calm": [
                            "🤫 bình tĩnh~",
                            "😶 bình tĩnh nha~",
                            "🕊️ bình tĩnh nè~"
                      ],
                      "shy": [
                            "👀 hơi ngại~",
                            "😐 hơi ngại nha~",
                            "🐜 hơi ngại nè~"
                      ],
                      "grumpy": [
                            "😩 chán quá~",
                            "🌬️ chán quá nha~",
                            "😑 chán quá nè~"
                      ],
                      "energetic": [
                            "🤸 muốn chạy~",
                            "🚪 muốn chạy nha~",
                            "💣 muốn chạy nè~"
                      ]
                },
                "l2": {
                      "affectionate": [
                            "🔭 ở với mình!",
                            "👣 ở với mình nha!",
                            "🐕 ở với mình nè!"
                      ],
                      "playful": [
                            "⚡️ làm gì ta!",
                            "👆 làm gì ta nha!",
                            "📢 làm gì ta nè!"
                      ],
                      "calm": [
                            "💭 bình tĩnh!",
                            "🧘 bình tĩnh nha!",
                            "🍵 bình tĩnh nè!"
                      ],
                      "shy": [
                            "👉👈 hơi ngại!",
                            "🍃 hơi ngại nha!",
                            "🔇 hơi ngại nè!"
                      ],
                      "grumpy": [
                            "🌵 chán quá!",
                            "🛌 chán quá nha!",
                            "😤 chán quá nè!"
                      ],
                      "energetic": [
                            "🐜 muốn chạy!",
                            "💨 muốn chạy nha!",
                            "📢 muốn chạy nè!"
                      ]
                },
                "l3": {
                      "affectionate": [
                            "😿 ở với mình!!",
                            "🍂 ở với mình luôn!",
                            "😭 ở với mình quá!"
                      ],
                      "playful": [
                            "🤪 làm gì ta!!",
                            "😈 làm gì ta luôn!",
                            "🖌️ làm gì ta quá!"
                      ],
                      "calm": [
                            "🌑 bình tĩnh!!",
                            "🤐 bình tĩnh luôn!",
                            "😶 bình tĩnh quá!"
                      ],
                      "shy": [
                            "🤧 hơi ngại!!",
                            "😨 hơi ngại luôn!",
                            "🌃 hơi ngại quá!"
                      ],
                      "grumpy": [
                            "✋ chán quá!!",
                            "🙉 chán quá luôn!",
                            "👎 chán quá quá!"
                      ],
                      "energetic": [
                            "🌋 muốn chạy!!",
                            "🏃 muốn chạy luôn!",
                            "😫 muốn chạy quá!"
                      ]
                }
          },
          "sleepy": {
                "l1": {
                      "affectionate": [
                            "🥱 buồn ngủ~",
                            "🎶 buồn ngủ nha~",
                            "🧸 buồn ngủ nè~"
                      ],
                      "playful": [
                            "🥴 chơi thêm chút~",
                            "🙅 chơi thêm chút nha~",
                            "😪 chơi thêm chút nè~"
                      ],
                      "calm": [
                            "🫢 đi ngủ~",
                            "🛀 đi ngủ nha~",
                            "🍵 đi ngủ nè~"
                      ],
                      "shy": [
                            "😵 ngáp rồi~",
                            "🛌 ngáp rồi nha~",
                            "🤤 ngáp rồi nè~"
                      ],
                      "grumpy": [
                            "💡 im lặng đi~",
                            "🚫 im lặng đi nha~",
                            "💤 im lặng đi nè~"
                      ],
                      "energetic": [
                            "🚲 hết pin rồi~",
                            "🔋 hết pin rồi nha~",
                            "😵 hết pin rồi nè~"
                      ]
                },
                "l2": {
                      "affectionate": [
                            "🌈 buồn ngủ!",
                            "🐑 buồn ngủ nha!",
                            "💤 buồn ngủ nè!"
                      ],
                      "playful": [
                            "😴 chơi thêm chút!",
                            "🤥 chơi thêm chút nha!",
                            "💬 chơi thêm chút nè!"
                      ],
                      "calm": [
                            "🛌 đi ngủ!",
                            "🌙 đi ngủ nha!",
                            "🤫 đi ngủ nè!"
                      ],
                      "shy": [
                            "🛌 ngáp rồi!",
                            "😪 ngáp rồi nha!",
                            "🌛 ngáp rồi nè!"
                      ],
                      "grumpy": [
                            "👿 đừng đánh thức!",
                            "🐕 ..gừ gừ.",
                            "✋ nghỉ chút nhé."
                      ],
                      "energetic": [
                            "🔌 hết pin rồi!",
                            "📴 hết pin rồi nha!",
                            "🪫 hết pin rồi nè!"
                      ]
                },
                "l3": {
                      "affectionate": [
                            "🌠 buồn ngủ!!",
                            "💜 buồn ngủ luôn!",
                            "😴 buồn ngủ quá!"
                      ],
                      "playful": [
                            "💤 chơi thêm chút!!",
                            "🐴 chơi thêm chút luôn!",
                            "🛌 chơi thêm chút quá!"
                      ],
                      "calm": [
                            "🌑 đi ngủ!!",
                            "😶 đi ngủ luôn!",
                            "🔇 đi ngủ quá!"
                      ],
                      "shy": [
                            "👶 ngáp rồi!!",
                            "🌙 ngáp rồi luôn!",
                            "🫧 ngáp rồi quá!"
                      ],
                      "grumpy": [
                            "😑 im lặng đi!!",
                            "💢 muốn nghỉ thôi..",
                            "💤 im lặng đi quá!"
                      ],
                      "energetic": [
                            "🪫 hết pin rồi!!",
                            "☠️ hết pin rồi luôn!",
                            "😵 hết pin rồi quá!"
                      ]
                }
          },
          "sick": {
                "l1": {
                      "affectionate": [
                            "🤕 đau quá~",
                            "🩹 đau quá nha~",
                            "🤒 đau quá nè~"
                      ],
                      "playful": [
                            "😖 mệt ghê~",
                            "🤥 mệt ghê nha~",
                            "💥 mệt ghê nè~"
                      ],
                      "calm": [
                            "🌡️ nghỉ ngơi~",
                            "💫 nghỉ ngơi nha~",
                            "🛌 nghỉ ngơi nè~"
                      ],
                      "shy": [
                            "😣 khó chịu quá~",
                            "😢 khó chịu quá nha~",
                            "🤧 khó chịu quá nè~"
                      ],
                      "grumpy": [
                            "🤦 đừng làm phiền~",
                            "💢 đừng làm phiền nha~",
                            "😔 đừng làm phiền nè~"
                      ],
                      "energetic": [
                            "😵 không còn sức~",
                            "📉 không còn sức nha~",
                            "🥴 không còn sức nè~"
                      ]
                },
                "l2": {
                      "affectionate": [
                            "😭 đau quá!",
                            "🆘 đau quá nha!",
                            "💦 đau quá nè!"
                      ],
                      "playful": [
                            "🦆 mệt ghê!",
                            "📢 mệt ghê nha!",
                            "🌀 mệt ghê nè!"
                      ],
                      "calm": [
                            "🏥 nghỉ ngơi!",
                            "🔥 nghỉ ngơi nha!",
                            "💊 nghỉ ngơi nè!"
                      ],
                      "shy": [
                            "😓 khó chịu quá!",
                            "😨 khó chịu quá nha!",
                            "🚑 khó chịu quá nè!"
                      ],
                      "grumpy": [
                            "💊 đừng làm phiền!",
                            "😤 đừng làm phiền nha!",
                            "😣 đừng làm phiền nè!"
                      ],
                      "energetic": [
                            "🛌 không còn sức!",
                            "🏳️ không còn sức nha!",
                            "📉 không còn sức nè!"
                      ]
                },
                "l3": {
                      "affectionate": [
                            "😿 đau quá!!",
                            "👋 đau quá luôn!",
                            "🛌 đau quá quá!"
                      ],
                      "playful": [
                            "😵 mệt ghê!!",
                            "🎮 mệt ghê luôn!",
                            "❌ mệt ghê quá!"
                      ],
                      "calm": [
                            "📉 nghỉ ngơi!!",
                            "🌫️ nghỉ ngơi luôn!",
                            "🏥 nghỉ ngơi quá!"
                      ],
                      "shy": [
                            "🧎 khó chịu quá!!",
                            "😶 khó chịu quá luôn!",
                            "🏥 khó chịu quá quá!"
                      ],
                      "grumpy": [
                            "👿 đừng làm phiền!!",
                            "🤐 đừng làm phiền luôn!",
                            "😫 đừng làm phiền quá!"
                      ],
                      "energetic": [
                            "⏹️ không còn sức!!",
                            "⏸️ không còn sức luôn!"
                      ]
                }
          },
          "worried": {
                "l1": {
                      "affectionate": [
                            "😟 lo quá~",
                            "😥 lo quá nha~",
                            "⚠️ lo quá nè~"
                      ],
                      "playful": [
                            "😲 sao đây~",
                            "💦 sao đây nha~",
                            "😳 sao đây nè~"
                      ],
                      "calm": [
                            "🔍 bình tĩnh nào~",
                            "⁉️ bình tĩnh nào nha~",
                            "🛡️ bình tĩnh nào nè~"
                      ],
                      "shy": [
                            "😖 sợ quá~",
                            "👣 sợ quá nha~",
                            "😢 sợ quá nè~"
                      ],
                      "grumpy": [
                            "😒 khó chịu quá~",
                            "😠 khó chịu quá nha~",
                            "☁️ khó chịu quá nè~"
                      ],
                      "energetic": [
                            "🚨 nguy rồi~",
                            "☢️ nguy rồi nha~",
                            "🚧 nguy rồi nè~"
                      ]
                },
                "l2": {
                      "affectionate": [
                            "😨 lo quá!",
                            "🛡️ lo quá nha!",
                            "🥶 lo quá nè!"
                      ],
                      "playful": [
                            "🏃 sao đây!",
                            "📢 sao đây nha!",
                            "😱 sao đây nè!"
                      ],
                      "calm": [
                            "✋ bình tĩnh nào!",
                            "⛔️ bình tĩnh nào nha!",
                            "↩️ bình tĩnh nào nè!"
                      ],
                      "shy": [
                            "🫨 sợ quá!",
                            "📦 sợ quá nha!",
                            "🙈 sợ quá nè!"
                      ],
                      "grumpy": [
                            "😫 lo quá!",
                            "😤 chờ chút.",
                            "🦁 gừ gừ."
                      ],
                      "energetic": [
                            "🫢 nguy rồi!",
                            "🆘 nguy rồi nha!",
                            "🌪️ nguy rồi nè!"
                      ]
                },
                "l3": {
                      "affectionate": [
                            "😭 lo quá!!",
                            "💔 lo quá luôn!",
                            "🧟 lo quá quá!"
                      ],
                      "playful": [
                            "😭 sao đây!!",
                            "🚑 sao đây luôn!",
                            "🤱 sao đây quá!"
                      ],
                      "calm": [
                            "📉 bình tĩnh nào!!",
                            "🧩 bình tĩnh nào luôn!",
                            "❌ bình tĩnh nào quá!"
                      ],
                      "shy": [
                            "🥀 sợ quá!!",
                            "💦 sợ quá luôn!",
                            "😱 sợ quá quá!"
                      ],
                      "grumpy": [
                            "🚮 nguy thật!",
                            "😫 khó chịu quá luôn!",
                            "🤬 khó chịu quá quá!"
                      ],
                      "energetic": [
                            "🤯 nguy rồi!!",
                            "🧱 nguy rồi luôn!",
                            "🆘 nguy rồi quá!"
                      ]
                }
          },
          "angry": {
                "l1": {
                      "affectionate": [
                            "😤 giận rồi~",
                            "😞 giận rồi nha~",
                            "😒 giận rồi nè~"
                      ],
                      "playful": [
                            "👊 không vui đâu~",
                            "🥊 không vui đâu nha~",
                            "⚔️ không vui đâu nè~"
                      ],
                      "calm": [
                            "😔 dừng lại~",
                            "✋ dừng lại nha~",
                            "😐 dừng lại nè~"
                      ],
                      "shy": [
                            "🥺 bực quá~",
                            "😢 bực quá nha~",
                            "😿 bực quá nè~"
                      ],
                      "grumpy": [
                            "👉 không~",
                            "🤫 im đi~",
                            "🌩️ dừng lại~"
                      ],
                      "energetic": [
                            "😡 tức quá~",
                            "🥋 tức quá nha~",
                            "🦁 tức quá nè~"
                      ]
                },
                "l2": {
                      "affectionate": [
                            "😠 giận rồi!",
                            "🙅 giận rồi nha!",
                            "🤥 giận rồi nè!"
                      ],
                      "playful": [
                            "💣 không vui đâu!",
                            "🐊 không vui đâu nha!",
                            "🦖 không vui đâu nè!"
                      ],
                      "calm": [
                            "⚠️ dừng lại!",
                            "🚫 dừng lại nha!",
                            "🛑 dừng lại nè!"
                      ],
                      "shy": [
                            "😭 bực quá!",
                            "👋 bực quá nha!",
                            "😣 bực quá nè!"
                      ],
                      "grumpy": [
                            "👿 bực quá!",
                            "🙈 đừng nhìn nha!",
                            "🗯️ giận rồi nè!"
                      ],
                      "energetic": [
                            "👺 tức quá!",
                            "💢 bực quá nha!",
                            "🥊 tức quá nè!"
                      ]
                },
                "l3": {
                      "affectionate": [
                            "😤 giận rồi!!",
                            "🙈 giận rồi luôn!",
                            "💢 giận rồi quá!"
                      ],
                      "playful": [
                            "🚂 rầm rầm!!",
                            "🌪️ giận lắm luôn!",
                            "😈 không vui đâu quá!"
                      ],
                      "calm": [
                            "😑 dừng lại!!",
                            "🛑 dừng lại luôn!",
                            "🔚 dừng lại quá!"
                      ],
                      "shy": [
                            "🌊 bực quá!!",
                            "😭 bực quá luôn!",
                            "👶 bực quá quá!"
                      ],
                      "grumpy": [
                            "🌋 giận lắm!!",
                            "💨 tránh ra luôn!",
                            "😫 hứ quá!"
                      ],
                      "energetic": [
                            "🔥 giận lắm!!",
                            "🔨 áaa luôn!",
                            "🤬 tức quá quá!"
                      ]
                }
          }
    },


  },
  abandonment: {
    danger: 'Cần được chăm sóc!',
    critical: 'Tình trạng nguy cấp!',
    leaving: 'Sắp rời đi!',
    abandoned: 'R.I.P. Jello đã rời xa bạn...',
  },
  settings: {
    title: 'Cài đặt',
    sound: {
      title: 'Âm thanh',
      description: 'Điều chỉnh âm thanh',
      bgm: 'Nhạc nền',
      sfx: 'Hiệu ứng âm thanh',
      on: 'ON',
      off: 'OFF',
    },
    language: {
      title: 'Ngôn ngữ',
      description: 'Chọn ngôn ngữ',
      selected: 'Đã chọn',
    },
    mode: {
      title: 'Chế độ',
      game: 'Game (Mặc định)',
      learn: 'Learn',
    },
    admin: {
      title: 'Quản trị',
      gallery: 'Thư viện',
      stats: 'Thống kê',
    },
    cloudSave: 'Lưu đám mây',
    logout: 'Đăng xuất',
    saveStatus: {
      idle: 'Lưu đám mây',
      saving: 'Đang lưu...',
      success: 'Đã lưu!',
      error: 'Thất bại',
      cooldown: 'Đợi {{time}} giây',
    },
  },
  encyclopedia: {
    title: 'Bộ sưu tập Jello',
    home: 'Trang chủ',
    species: 'Loài',
    stage: 'Giai đoạn {{stage}}',
    hidden: 'CHƯA MỞ',
    legendary: {
      title: 'Tiến hóa huyền thoại',
      prefix: 'Bạn cần',
      suffix: 'để mở khóa dạng này!',
    },
  },
  auth: {
    promo: {
      title: 'Lưu Jello của bạn!',
      desc: 'Để tiến hóa lên giai đoạn 2, bạn cần lưu tiến trình. <highlight>Đăng ký ngay</highlight> để giữ Jello của bạn an toàn mãi mãi!',
      later: 'Để sau',
    },
    login: {
      title: 'Đăng nhập',
      subtitle: 'Jello đáng yêu của bạn đang chờ!',
      needAccount: 'Bạn cần một tài khoản?',
      email: 'Email',
      emailPlaceholder: 'Nhập email',
      password: 'Mật khẩu',
      passwordPlaceholder: 'Nhập mật khẩu',
      or: 'HOẶC',
      google: 'Tiếp tục với Google',
      signup: 'Đăng ký bằng email',
      action: 'Đăng nhập',
    },
    signup: {
      title: 'Đăng ký',
      subtitle: 'Bước vào thế giới bí mật của GroGroJello',
      emailLabel: 'Email',
      emailPlaceholder: 'Nhập email',
      passwordLabel: 'Mật khẩu',
      passwordPlaceholder: 'Nhập mật khẩu (từ 6 ký tự)',
      confirmPasswordLabel: 'Xác nhận mật khẩu',
      confirmPasswordPlaceholder: 'Nhập lại mật khẩu',
      haveAccount: 'Đã có tài khoản?',
      loginLink: 'Đăng nhập',
      passwordMismatch: 'Mật khẩu không khớp.',
      success: 'Đăng ký thành công!',
      action: 'Đăng ký',
      consentPrefix: 'Bằng việc đăng ký, bạn đồng ý với ',
      termsLink: 'Điều khoản Dịch vụ',
      privacyLink: 'Chính sách Quyền riêng tư',
      consentConnector: ' và ',
      consentSuffix: '.',
    },
    errors: {
      default: 'Đăng nhập thất bại.',
      invalidCredential: 'Email hoặc mật khẩu không đúng.',
      tooManyRequests: 'Thử quá nhiều lần. Vui lòng thử lại sau.',
      googleFailed: 'Đăng nhập Google thất bại.',
      registrationFailed: 'Đăng ký thất bại.',
      emailInUse: 'Email đã được sử dụng.',
      weakPassword: 'Mật khẩu phải có ít nhất 6 ký tự.',
      invalidEmail: 'Định dạng email không hợp lệ.',
    },
    logging_in: 'Đang đăng nhập...',
    signing_up: 'Đang đăng ký...',
    googleLogoAlt: 'Biểu trưng Google',
  },
  play: {
    title: 'Chơi & Học',
    home: 'Trang chủ',
    controls: {
      title: 'Cùng chơi nào!',
      expand: 'Mở rộng',
      collapse: 'Thu gọn',
      level: 'Cấp độ',
    },
    modes: {
      adventure: 'PHIÊU LƯU',
      genius: 'THIÊN TÀI',
    },
    sections: {
      funMath: {
        title: 'Toán vui nhộn',
        desc: 'Nhiệm vụ toán học dạng game',
      },
      training: {
        title: 'Rèn luyện',
        desc: 'Mô-đun luyện tập',
      },
      play: {
        desc: 'Thưởng thức các trò chơi vui nhộn',
      },
      genius: {
        title: 'Tính toán thiên tài',
        desc: "Bí quyết tính nhanh của thiên tài",
      },
    },
    retro: {
      systemReady: 'Hệ thống sẵn sàng',
      selectCartridge: 'Chọn băng trò chơi',
      swipeOrTap: 'Vuốt → Chạm → Bắt đầu!',
      inserting: 'Đang lắp...',
      nowLoading: 'Đã tải xong',
      power: 'Nguồn',
      insert: 'Lắp vào',
      start: 'Bắt đầu',
      eject: 'Lấy ra',
      insertPackFirst: 'Hãy lắp băng trò chơi trước',
      reset: 'Đặt lại',
    },
    categories: {
      brain: 'TƯ DUY',
      math: 'TOÁN',
      science: 'CÀI ĐẶT',
      play: 'CHƠI',
    },
    worlds: {
      level1: 'Thế Giới Nước',
      level2: 'Thế Giới Rừng',
      level3: 'Thế Giới Sa Mạc',
    },
    comingSoon: {
      nextAdventure: 'Cuộc phiêu lưu tiếp theo',
      title: 'Sắp ra mắt',
    },
    game: {
      playNow: 'Chơi ngay',
      reset: 'Thử lại',
      noGames: 'Sắp có thêm trò chơi!',
      unlock: {
        reason: 'Hoàn thành thuần thục {{game}} để mở khóa',
      },
      gameTitles: {
        tailRunner: 'KẺ CHẠY ĐUÔI',
        jelloKnight: 'HIỆP SĨ JELLO',
        groGroLand: 'VÙNG ĐẤT GROGRO',
      },
      gameSubtitles: {
        tailRunner: 'Chạy cùng Jello của bạn.',
        jelloKnight: 'Sinh tồn và giải cứu tất cả.',
        groGroLand: 'Mở rộng vùng đất Jello của bạn.',
      },
    },
  },
  game: {
    playNow: 'Chơi ngay',
    reset: 'Thử lại',
    noGames: 'Sẽ sớm có thêm trò chơi mới!',
    unlock: {
      reason: 'Hoàn thành bậc thầy của {{game}} để mở khóa',
    },
    gameTitles: {
      tailRunner: 'KẺ CHẠY ĐUÔI',
      jelloKnight: 'HIỆP SĨ JELLO',
      groGroLand: 'VÙNG ĐẤT GROGRO',
    },
    gameSubtitles: {
      tailRunner: 'Chạy cùng Jello của bạn.',
      jelloKnight: 'Sinh tồn và giải cứu tất cả.',
      groGroLand: 'Mở rộng vùng đất Jello của bạn.',
    },
  },
  games: {
    'math-fishing-count': fishingCountVi,
    'tenframe-number': tenFrameNumberLocale,
    'ten-frame-add': tenFrameAddLocale,
    'mole-whack': moleWhackLocale,
    'math-round-counting': roundCountingVi,
    'math-number-hive': numberHiveVi,
    'math-fruit-slice': fruitSliceVi,
    'math-number-balance': numberBalanceVi,
    'math-archery': mathArcheryVi,
    'jello-feeding': jelloFeedingVi,
    'math-compare-critters': compareCrittersVi,
    frontAddition: frontAdditionVi,
    frontSubtraction: frontSubtractionVi,
    backMultiplication: backMultiplicationVi,
    'ten-frame-count': tenFrameCountVi,
    'pinwheel-pop': mathPinwheelVi,
    'shape-sum-link': shapeSumLinkVi,
    'fruit-box': fruitBoxVi,
    'ice-stacking': iceStackingVi,
        'floor-tiler': floorTilerVi,
        'frog-jump': frogJumpVi,
        'chip-cashier': chipCashierVi,
        'cell-clone': cellCloneVi,
        'fair-share': fairShareLocale,
        'donut-shop': donutShopLocale,
        'chocolate-split': chocolateSplitLocale,
        'sign-shifter': signShifterLocale,
        'scorpion-king': scorpionKingLocale,
        'pizza-pizza': pizzaPizzaLocale,
        'river-crossing': riverCrossingLocale,
        'neon-matrix': neonMatrixVi,
        'three-leaf-clover': threeLeafCloverLocale,
        'hex-hive-six': hexHiveSixLocale,
        'flight-calendar': flightCalendarLocale,
    'ten-frame-pop': tenFramePopLocale,
    'beginner-wizard': beginnerWizardVi,
    'constellation-finder': constellationFinderVi,
    'troll-attack': trollAttackVi,
    'deep-sea-dive': deepSeaDiveVi,
    'signal-hunter': signalHunterVi,
    'block-tower': blockTowerVi,
    sharpshooter: sharpshooterVi,
    'color-link': colorLinkVi,
    'pair-up-twin': pairUpTwinVi,
    'wild-link': wildLinkVi,
    'pair-up-connect': pairUpConnectVi,
    'animal-banquet': animalBanquetVi,
    'color-mix': colorMixVi,
    'math-level2-ufo-invasion': ufoInvasionVi,
    'maze-escape': mazeEscapeVi,
    'maze-hunter': mazeHunterVi,
    'tic-tac-toe': ticTacToeVi,
    omok: omokVi,
    cargoTrain: cargoTrainVi,
    rocketLauncher: rocketLauncherVi,
    
    
    
    'math-lock-opening': lockOpeningLocale,
    'math-magic-potion': magicPotionLocale,
    
    
    tags: {
      counting: 'Đếm số',
      sequence: 'Thứ tự',
      numberSense: 'Cảm nhận số',
      addition: 'Cộng',
      subtraction: 'Trừ',
      partWhole: 'Cấu tạo số',
      mixedOps: 'Phép tính hỗn hợp',
      speedMath: 'Toán nhanh',
      mentalMath: 'Tính nhẩm',
      spatial: 'Tư duy không gian',
      observation: 'Quan sát',
      categorization: 'Phân loại',
      workingMemory: 'Trí nhớ làm việc',
      association: 'Liên kết',
      concentration: 'Tập trung',
      strategy: 'Chiến lược',
      memory: 'Ghi nhớ',
      logic: 'Logic',
      comparison: 'So sánh',
      division: 'Chia',
      multiplication: 'Nhân',
    },
    mission: {
      challenge: 'Thử thách! ({{current}}/{{total}})',
      challenge10: 'Thử thách! ({{current}}/{{total}})',
    },
    medal: {
      bronze: '🥈 Đạt Bạc sau {{count}} lần chơi!',
      silver: '🥇 Đạt Vàng sau {{count}} lần chơi!',
      gold: 'Tuyệt vời! Đã làm chủ! 🎉',
    },
  },
  giftBox: {
    nicknameTitle: 'Rất vui được gặp bạn!',
    nicknamePlaceholder: 'Nhập tên Jello của bạn',
    saveError: 'Không thể lưu tên. Vui lòng thử lại.',
    startButton: 'Bắt đầu!',
    saving: 'Đang lưu...',
    tapHint: 'Chạm nhiều lần nhé!',
    holdHint: 'Nhấn giữ để nở',
  },
  dailyRoutine: {
    title: 'Nhiệm vụ hằng ngày',
    claim: 'Nhận thưởng',
    claiming: 'Đang nhận...',
    claimed: 'Hoàn thành hôm nay',
    done: 'Hoàn thành',
    inProgress: 'Đang tiến hành',
    task: {
      studyStars: 'Hoàn thành 5 trò chơi',
      petTouch: 'Vuốt ve jello 5 lần',
      feedCategory: 'Cho ăn {{category}} 3 lần',
      brushTeeth: 'Đánh răng 2 lần',
      shower: 'Tắm 2 lần',
      cleanPoop: 'Dọn poop 2 lần',
      cleanBug: 'Bắt côn trùng 1 lần',
      sleep: 'Cho đi ngủ 1 lần',
    },
    foodCategory: {
      fruit: 'trái cây',
      vegetable: 'rau',
      bakery: 'bánh',
      meal: 'thức ăn',
      dessert: 'tráng miệng',
    },
    reward: {
      title: 'Thưởng nhiệm vụ hằng ngày',
    },
    error: {
      loginRequired: 'Cần đăng nhập để nhận thưởng nhiệm vụ hằng ngày.',
      alreadyClaimed: 'Phần thưởng nhiệm vụ hằng ngày hôm nay đã được nhận.',
      claimFailed: 'Không thể nhận phần thưởng nhiệm vụ hằng ngày.',
    },
  },
  train: {
    reward: {
      glo: 'GLO',
      dud: 'Không có gì!',
      confirm: 'Xác nhận',
    },
  },
} as const;

export type TranslationKeys = typeof viVN;
